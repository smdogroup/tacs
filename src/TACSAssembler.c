...
/*
  Computes the sensitivities of the supplied function to the array of
  state variables. This function uses a central difference if the
  real code is compiled, and a complex step approximation if the
  complex version of the code is used.
  
  input:
  func:            the function to get the design variable sesitivity
  q:               the array of design variables
  dh:              the step size to use

  output:
  dfdq:            the sensitivity of function to state variables (q only)
*/
void TACSAssembler::getApproxFunctionSVSens( TACSFunction *func, BVec *dfdq, 
					     double dh ) {
  
  if (!meshFinalizedFlag){ // needed?
    fprintf(stderr, "[%d] Cannot call testFunction() before finalize()\n", 
	    mpiRank);
    return;
  }
  
  // Fetch the original q  
  BVec * q = createVec();
  q->incref();
  getVariables(q);
 
  // Find the number of state variables
  int num_state_vars;
  q->getSize(&num_state_vars);

  // Space for perturbed q's
  BVec *qtemp = createVec();
  qtemp->incref();  
  qtemp->copyValues(q);
  
  TacsScalar *qvals;
  qtemp->getArray(&qvals);

  TacsScalar *dfdq_vals;
  dfdq->getArray(&dfdq_vals);

  TacsScalar qsave = 0.0;
  TacsScalar fd = 0.0, fval1 = 0.0, fval2 = 0.0;
  ...
  for ( int k = 0; k < num_state_vars; k++ ){
    ...
    // Perturb the imaginary part of kth-state variable
    qvals[k] = qsave + TacsScalar(0.0, dh);

    // Set the perturbed state into TACS
    setVariables(qtemp);
    ...
    dfdq_vals[k] = ImagPart(fd)/dh;
    
    // Restrore the original value
    qvals[k] = qsave;
  }
#else
  // Compute the function at the point x + dh
  for ( int k = 0; k < num_state_vars; k++ ){
    // Save the original value
    qsave = qvals[k];

    // Perturb q[k] as q[k] + dh
    qvals[k] = qsave + dh;
    ...
    // Evalute the function
    evalFunctions(&func, 1, &fval2);
    
    // Approximate the derivative using central differences
    dfdq_vals[k] = 0.5*(fval1 - fval2)/dh;    
  ...
  // Make sure TACS contains the original SV values
  setVariables(q);
...
/*
  Computes the sensitivities of the supplied function to the array of
  design variables. This function uses a central difference if the
  real code is compiled, and a complex step approximation if the
  complex version of the code is used.
  ...
  x:               the array of design variables
  dh:              the step size to use

  output:
  dfdx:            the sensitivity of function to design variables
*/
void TACSAssembler::getApproxFunctionDVSens( TACSFunction ** funcs, 
					     int num_funcs,
					     int num_design_vars, 
					     TacsScalar * dfdx, 
					     double dh ){
  ...
  // Retrieve the DV vector already set into TACS
  ...
  TacsScalar *fd = new TacsScalar[num_funcs * num_design_vars];
  TacsScalar *fval1 = new TacsScalar[num_funcs * num_design_vars];
  TacsScalar *fval2 = new TacsScalar[num_funcs * num_design_vars];
  
#ifdef TACS_USE_COMPLEX
  for ( int k = 0; k < num_design_vars; k++ ){
    // Perturb the imaginary part of kth-design variable
    xtemp[k] = x[k] + TacsScalar(0.0, dh);

    // Set the DV into TACS
    setDesignVars(xtemp, num_design_vars);

    // Evaluate the function 
    evalFunctions(funcs, num_funcs, fd);
    
    // Evaluate the approximate derivative
    for ( int j = 0; j < num_funcs; j++ ){
      dfdx[k+j*num_design_vars] = ImagPart(fd[j])/dh;
    }
  ...
  for ( int k = 0; k < num_design_vars; k++ ){
    ...
    // Evaluate the function 
    evalFunctions(funcs, num_funcs, fval2);
    ...
    // Evaluate the function 
    evalFunctions(funcs, num_funcs, fval1);

    // Evaluate the approximate derivative
    for ( int j = 0; j < num_funcs; j++ ){
      dfdx[k+j*num_design_vars] = 0.5*(fval2[j] - fval1[j])/dh;
    }
  ...
  // Make sure TACS contains the original DV values
