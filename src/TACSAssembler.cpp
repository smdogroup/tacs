...
void TACSAssembler::computePanelDimensions() {
  // only allows TACSGPBladeStiffenedShellConstitutive const. objects in the
  // element (so maybe throw error if not?, or a warning and skip?)
  //    or only do this over certain element type, constitutive type elements
  //    and skip otherwise..
  ...
  for (int icomp = 0; icomp < getNumComponents(); icomp++) {
    ...
    for (int i = 0; i < numElements; i++) {
      if (elements[i]->getComponentNum() == icomp &&
          element[i]->needsPanelDimensions()) {
        // get element Xpts
        int ptr = elementNodeIndex[i];
        int len = elementNodeIndex[i + 1] - ptr;
        const int *nodes = &elementTacsNodes[ptr];
        ...
        elements[i]->evalPointQuantity(i, TACS_ELEMENT_DENSITY_MOMENT, 0.0, n,
                                       pt, elemXpts, vars, dvars, ddvars, detXd,
                                       &local_centroidint);
        // still need a way to get local_area, maybe add a new section where
        // density is 1 and we compute the moment differently
    ...
    MPI_Allreduce(&local_centroidint, &global_centroidint, 3, TACS_MPI_TYPE,
                  MPI_SUM, tacs_comm);
    MPI_Allreduce(&local_area, &global_area, 1, TACS_MPI_TYPE, MPI_SUM,
                  tacs_comm);
    ...
    for (int i = 0; i < numElements; i++) {
      if (elements[i]->getComponentNum() == icomp &&
        ...
        elements[i]->evalPointQuantity(i, TACS_ELEMENT_MOMENT_OF_INERTIA, 0.0,
                                       n, pt, elemXpts, vars, dvars, ddvars,
                                       detXd, &local_inertia);
    ...
    MPI_Allreduce(&local_inertia, &global_inertia, 6, TACS_MPI_TYPE, MPI_SUM,
                  tacs_comm);

    // compute the two principal axes with the middle and lowest diagonal
    // inertias
    ...
    // use KS function to get positive difference in max and min coordinates
    // along length and width direction
    TacsScalar min[2] = {}, max[2] = {};
    TacsScalar refAxis[3];
    for (int i = 0; i < numElements; i++) {
      if (elements[i]->getComponentNum() == icomp &&
          element[i]->needsPanelDimensions()) {
        // for each axis direction, e.g. min[0] += exp(-ks*dotprod(axis1,
        // xyz_coord))
