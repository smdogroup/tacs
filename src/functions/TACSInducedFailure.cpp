  ...
  for ( int i = 0; i < element->getNumQuadraturePoints(); i++ ){
    ...
    if (count >= 1){
      ...
      if (normType == POWER){
        ...
        dfdq = weight*detXd*((1.0 + P)*g*failDenom -
        ...
        dfdq = weight*detXd*((2.0 + P)*fail*g*failDenom -
                             P*failNumer)*fp/(g*failDenom*failDenom);
      }
      else if (normType == DISCRETE_POWER_SQUARED){
        TacsScalar g = fail/maxFail;
        ...
        dfdq = weight*detXd*(((1.0 + P*fail)*failDenom -
        ...
        dfdq = weight*detXd*(((2.0 + P*fail)*fail*failDenom -
                              P*maxFail*failNumer)*efp)/(failDenom*failDenom);
      }
      else if (normType == DISCRETE_EXPONENTIAL_SQUARED){
        TacsScalar efp = exp(P*(fail - maxFail));
