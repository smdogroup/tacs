#ifndef TACS_INDUCED_BUCKLING_H
#define TACS_INDUCED_BUCKLING_H
...
class InducedBuckling : public TACSFunction {
 public:
  ...
  InducedBuckling( TACSAssembler * _tacs, int _elementNums[], 
		  int _numElements, double _P );
  InducedBuckling( TACSAssembler * _tacs, double _P );
  ~InducedBuckling();
  ...
  // Set parameters to control how the induced buckling functions are evaluated
  // --------------------------------------------------------------------------
 ...
 private:
  ...
  TacsScalar maxBuckling; // The maximum failure function at a Gauss point
  TacsScalar bucklingNumer, bucklingDenom; // The numerator and denominator
