...
*/
TacsScalar TACSKSDisplacement::getMaximumDisplacement(){
  return maxDisp;
}
    ...
    if (count >= 1){
      if (ftype == TACSFunction::INITIALIZE){
        ...
        if (ksType == DISCRETE){
          ...
          TacsScalar fpow = pow(fabs(TacsRealPart(dispProj/maxDisp)), ksWeight);
  ...
  for ( int i = 0; i < element->getNumQuadraturePoints(); i++ ){
    ...
    if (count >= 1){
      ...
      if (ksType == DISCRETE){
        ...
        TacsScalar fpow = pow(fabs(TacsRealPart(dispProj/maxDisp)), ksWeight-2.0);
