...
/*
  Constructor for sliding pivot contraint taking Gibbs vectors as
  inputs. A refers to bodyA and B refers to bodyB.
 ...
 */
 ...
 /*
   Destuctor for the sliding pivot constraint
 */
 TACSSlidingPivotConstraint::~TACSSlidingPivotConstraint(){
   bodyA->decref();
  if(bodyB){ bodyB->decref(); }
  point->decref();
...
const char *TACSSlidingPivotConstraint::elem_name = "TACSSlidingPivotConstraint";

/*
  Returns the number of nodes based on the constraint nature
*/
int TACSSlidingPivotConstraint::numNodes(){
  if(bodyA && bodyB){
    return 3;
  } 
  else {
...
*/
void TACSSlidingPivotConstraint::updatePoints( int init_e ){
  // Retrieve the coordinates of the joint point in the global frame
  const TacsScalar *pt;
  point->getVector(&pt);
  ...
  // the purpose of optimization, this direction is fixed at
  // initialization.
  // Retrieve the sliding direction in global frame
  const TacsScalar *rev;
  eAVec->getVector(&rev);
...
/*
  Compute the kinetic and potential energy within the element
*/
void TACSSlidingPivotConstraint::computeEnergies( double time,
                                                 TacsScalar *_Te, 
                                                 TacsScalar *_Pe,
                                                 const TacsScalar Xpts[],
                                                 const TacsScalar vars[],
...
/*
  Compute the residual of the governing equations
*/
void TACSSlidingPivotConstraint::addResidual( double time, TacsScalar res[],
                                              const TacsScalar Xpts[],
                                              const TacsScalar vars[],
                                              const TacsScalar dvars[],
                                              const TacsScalar ddvars[] ){
  ...
  // Apply the reaction forces lam_j*dGi/dq_j
  resA[0] += lam[0]*eB1[0] + lam[1]*eB2[0];
  resA[1] += lam[0]*eB1[1] + lam[1]*eB2[1];
  resA[2] += lam[0]*eB1[2] + lam[1]*eB2[2];
}
...
/*
  Get the design variable values associated with the joint location
*/
void TACSSlidingPivotConstraint::getDesignVars( TacsScalar dvs[], int numDVs ){
  point->getDesignVars(dvs, numDVs);
}
...
/*
  Constructor for prismatic contraint taking Gibbs vectors as
  inputs. A refers to bodyA and B refers to bodyB. This is a one
  ...
  rev   : the cylindrical direction in global frame
*/
TACSPrismaticConstraint::TACSPrismaticConstraint( TACSRigidBody *_bodyA, 
                                                  TACSRigidBody *_bodyB, 
                                                  TACSGibbsVector *_point, 
                                                  TACSGibbsVector *_eAVec ){
  // Copy over the input arguments
...
/*
  ...
  bodyA : pointer to bodyA
  point : the position of the joint from the global reference point
  rev   : the  direction in global frame
*/
TACSPrismaticConstraint::TACSPrismaticConstraint( TACSRigidBody *_bodyA, 
                                                  TACSGibbsVector *_point, 
                                                  TACSGibbsVector *_eAVec ){
...
/*
  Destuctor for the prismatic constraint
*/
TACSPrismaticConstraint::~TACSPrismaticConstraint(){
  bodyA->decref();
  if(bodyB){ bodyB->decref(); }
  point->decref();
...
const char *TACSPrismaticConstraint::elem_name = "TACSPrismaticConstraint";

/*
  Returns the number of nodes based on the constraint nature
*/
int TACSPrismaticConstraint::numNodes(){
  if(bodyA && bodyB){
    return 3;
  } 
  else {
...
/*
  ...
  points of bodies A and B in the global (inertial) reference frame.
*/
void TACSPrismaticConstraint::updatePoints( int init_e ){
  // Retrieve the coordinates of the joint point in the global frame
  const TacsScalar *pt;
  point->getVector(&pt);
...
/*
  Compute the kinetic and potential energy within the element
*/
void TACSPrismaticConstraint::computeEnergies( double time,
                                                 TacsScalar *_Te, 
                                                 TacsScalar *_Pe,
                                                 const TacsScalar Xpts[],
                                                 const TacsScalar vars[],
...
/*
  Compute the residual of the governing equations
*/
void TACSPrismaticConstraint::addResidual( double time, TacsScalar res[],
                                           const TacsScalar Xpts[],
                                           const TacsScalar vars[],
                                           const TacsScalar dvars[],
                                           const TacsScalar ddvars[] ){
...
/*
  Set the design variable values
*/
void TACSPrismaticConstraint::setDesignVars( const TacsScalar dvs[], 
                                               int numDVs ){
  point->setDesignVars(dvs, numDVs);
  updatePoints();
}
...
/*
  Get the design variable values associated with the joint location
*/
void TACSPrismaticConstraint::getDesignVars( TacsScalar dvs[], int numDVs ){
  point->getDesignVars(dvs, numDVs);
}
