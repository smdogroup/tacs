...
*/
static inline void compute_jacobian_zeta_sens( TacsScalar dzJinv[],
                                               const TacsScalar Jinv[],
                                               const TacsScalar dJinv[],
                                               const TacsScalar n_xi[],
                                               const TacsScalar dn_xi[],
                                               const TacsScalar n_eta[],
  ...
  dtp[0] = - (dJinv[0]*n_xi[0] + dJinv[1]*n_eta[0] +
              Jinv[0]*dn_xi[0] + Jinv[1]*dn_eta[0]);
  dtp[3] = - (dJinv[3]*n_xi[0] + dJinv[4]*n_eta[0] +
              Jinv[3]*dn_xi[0] + Jinv[4]*dn_eta[0]);
  dtp[6] = - (dJinv[6]*n_xi[0] + dJinv[7]*n_eta[0] +
              Jinv[6]*dn_xi[0] + Jinv[7]*dn_eta[0]);

  dtp[1] = - (dJinv[0]*n_xi[1] + dJinv[1]*n_eta[1] +
              Jinv[0]*dn_xi[1] + Jinv[1]*dn_eta[1]);
  dtp[4] = - (dJinv[3]*n_xi[1] + dJinv[4]*n_eta[1] +
              Jinv[3]*dn_xi[1] + Jinv[4]*dn_eta[1]);
  dtp[7] = - (dJinv[6]*n_xi[1] + dJinv[7]*n_eta[1] +
              Jinv[6]*dn_xi[1] + Jinv[7]*dn_eta[1]);

  dtp[2] = - (dJinv[0]*n_xi[2] + dJinv[1]*n_eta[2] +
              Jinv[0]*dn_xi[2] + Jinv[1]*dn_eta[2]);
  dtp[5] = - (dJinv[3]*n_xi[2] + dJinv[4]*n_eta[2] +
              Jinv[3]*dn_xi[2] + Jinv[4]*dn_eta[2]);
  dtp[8] = - (dJinv[6]*n_xi[2] + dJinv[7]*n_eta[2] +
              Jinv[6]*dn_xi[2] + Jinv[7]*dn_eta[2]);
...
*/
static inline void transform_jacobian_product( TacsScalar tx[],
                                               TacsScalar ztx[],
                                               const TacsScalar t[],
                                               const TacsScalar Jinv[],
...
*/
static inline void transform_jacobian_product_sens( TacsScalar dtx[],
                                                    TacsScalar dztx[],
                                                    const TacsScalar t[],
                                                    const TacsScalar dt[],
                                                    const TacsScalar Jinv[],
...
*/
static inline void cross_product( TacsScalar n[],
...
*/
static inline void cross_product_sens( TacsScalar dn[],
                                       const TacsScalar A[],
                                       const TacsScalar B[],
                                       const TacsScalar dA[],
                                       const TacsScalar dB[] ){
...
*/
static inline void normalize_vector_sens( TacsScalar dn[],
                                          TacsScalar norm,
                                          const TacsScalar n[] ){
  if (norm != 0.0){
    TacsScalar norm_inv = 1.0/norm;
...
static inline void normalize_vector_sens( TacsScalar dn[],
  ...
  if (norm != 0.0){
    TacsScalar norm_inv = 1.0/norm;
    TacsScalar dnorm_inv =
      - norm_inv*norm_inv*norm_inv*(r[0]*dr[0] + r[1]*dr[1] + r[2]*dr[2]);
...
*/
static inline void normalize_vector_second_sens( TacsScalar dns[],
                                                 TacsScalar norm,
                                                 const TacsScalar r[],
                                                 const TacsScalar dr[],
                                                 const TacsScalar rs[],
                                                 const TacsScalar drs[] ){
  if (norm != 0.0){
    TacsScalar norm_inv = 1.0/norm;
    ...
    TacsScalar norm_invs =
      - norm_inv*norm_inv*norm_inv*(r[0]*rs[0] + r[1]*rs[1] + r[2]*rs[2]);

    TacsScalar dnorm_invs =
      - norm_inv*norm_inv*norm_inv*(r[0]*drs[0] + r[1]*drs[1] + r[2]*drs[2] +
                                    rs[0]*dr[0] + rs[1]*dr[1] + rs[2]*dr[2]) -
      (3.0*dnorm_inv)*norm_inv*norm_inv*(r[0]*rs[0] + r[1]*rs[1] + r[2]*rs[2]);

    dns[0] = (norm_invs*dr[0] + norm_inv*drs[0] +
              dnorm_invs*r[0] + dnorm_inv*rs[0]);

    dns[1] = (norm_invs*dr[1] + norm_inv*drs[1] +
              dnorm_invs*r[1] + dnorm_inv*rs[1]);

    dns[2] = (norm_invs*dr[2] + norm_inv*drs[2] +
              dnorm_invs*r[2] + dnorm_inv*rs[2]);
  }
  else {
...
*/
TacsScalar compute_transform( TacsScalar t[],
                              TacsScalar tx[],
                              TacsScalar ztx[],
                              TacsScalar n[],
                              TacsScalar n_xi[],
...
*/
TacsScalar compute_transform_sens( TacsScalar dh[],
                                   TacsScalar t[], TacsScalar dt[],
                                   TacsScalar tx[], TacsScalar dtx[],
                                   TacsScalar ztx[], TacsScalar dztx[],
                                   TacsScalar n[], TacsScalar dn[],
                                   TacsScalar n_xi[], TacsScalar dn_xi[],
                                   TacsScalar n_eta[], TacsScalar dn_eta[],
                                   TacsScalar Xd[], const TacsScalar Xdd[],
                                   const double Na[], const double Nb[],
                                   const double Naa[], const double Nab[],
                                   const double Nbb[],
                                   int num_nodes ){
  // n = Xd[0] x Xd[3]
  // |   i     j      k    |
  ...
  for ( int k = 0; k < num_nodes; k++ ){
    for ( int kk = 0; kk < 3; kk++ ){ // Cycle through the dimensions
      ...
      if (kk == 0){
        drn_xi[0] = 0.0;
        drn_xi[1] = Xd[2]*Nab[k] + Xdd[2]*Nb[k] - Na[k]*Xdd[5] - Naa[k]*Xd[5];
        drn_xi[2] = Na[k]*Xdd[4] + Naa[k]*Xd[4] - Xd[1]*Nab[k] - Xdd[1]*Nb[k];
      }
      else if (kk == 1){
        drn_xi[0] = Na[k]*Xdd[5] + Naa[k]*Xd[5] - Xd[2]*Nab[k] - Xdd[2]*Nb[k];
        drn_xi[1] = 0.0;
      ...
      if (kk == 0){
        drn_eta[0] = 0.0;
        drn_eta[1] = Xd[2]*Nbb[k] + Xdd[5]*Nb[k] - Na[k]*Xdd[8] - Nab[k]*Xd[5];
        drn_eta[2] = Na[k]*Xdd[7] + Nab[k]*Xd[4] - Xd[1]*Nbb[k] - Xdd[4]*Nb[k];
      }
      else if (kk == 1){
        drn_eta[0] = Na[k]*Xdd[8] + Nab[k]*Xd[5] - Xd[2]*Nbb[k] - Xdd[5]*Nb[k];
        drn_eta[1] = 0.0;
      ...
      compute_jacobian_zeta_sens(dzJinv, Jinv, dJinv,
                                 n_xi, dn_xi, n_eta, dn_eta);
      ...
      transform_jacobian_product_sens(dtx, dztx,
                                      t, dt, Jinv, zJinv, dJinv, dzJinv);
...
/*
  ...
  frame (e1,e2,e3).
*/
TacsScalar compute_transform_refaxis_sens( TacsScalar * dh,
                                           TacsScalar t[], TacsScalar dt[],
                                           TacsScalar tx[], TacsScalar dtx[],
                                           TacsScalar ztx[], TacsScalar dztx[],
                                           TacsScalar n[], TacsScalar dn[],
...
static inline void transform_vector3d_sens( TacsScalar vec[],
  ...
  vec[1] = (dt[3]*r[0] + dt[4]*r[1] + dt[5]*r[2] +
            t[3]*dr[0] + t[4]*dr[1] + t[5]*dr[2]);
  vec[2] = (dt[6]*r[0] + dt[7]*r[1] + dt[8]*r[2] +
            t[6]*dr[0] + t[7]*dr[1] + t[8]*dr[2]);
}
...
static inline void normal_rot_bmat( TacsScalar r[], TacsScalar r_xi[],
                                    TacsScalar r_eta[],
                                    int ii, double N, double Na, double Nb,
                                    const TacsScalar n[],
                                    const TacsScalar n_xi[],
                                    const TacsScalar n_eta[] ){
  if (ii < 3){
...
*/
void linear_strain( TacsScalar strain[],
                    TacsScalar *rotz,
                    const TacsScalar Ux[],
  ...
  *rotz = (0.5*(Ud[2]*tx[0] + Ud[3]*tx[1] + r[1]*tx[2] -
  ...
  strain[2] = (Ud[0]*tx[3] + Ud[1]*tx[4] + r[0]*tx[5] +
               Ud[2]*tx[0] + Ud[3]*tx[1] + r[1]*tx[2]); // e_12

  // Compute the bending components of the strain
  strain[3] = (r_xi[0]*tx[0] + r_eta[0]*tx[1] +
               Ud[0]*ztx[0] + Ud[1]*ztx[1] + r[0]*ztx[2]);  // k_11
  strain[4] = (r_xi[1]*tx[3] + r_eta[1]*tx[4] +
               Ud[2]*ztx[3] + Ud[3]*ztx[4] + r[1]*ztx[5]);  // k_22

  strain[5] = (r_xi[0]*tx[3] + r_eta[0]*tx[4] +
               r_xi[1]*tx[0] + r_eta[1]*tx[1] +
               Ud[0]*ztx[3] + Ud[1]*ztx[4] + r[0]*ztx[5] +
               Ud[2]*ztx[0] + Ud[3]*ztx[1] + r[1]*ztx[2]); // k_12

  // Compute the shear terms
  strain[6] = (Ud[2]*tx[6] + Ud[3]*tx[7] + r[1]*tx[8] +
               Ud[4]*tx[3] + Ud[5]*tx[4] + r[2]*tx[5]); // e_23
  strain[7] = (Ud[0]*tx[6] + Ud[1]*tx[7] + r[0]*tx[8] +
               Ud[4]*tx[0] + Ud[5]*tx[1] + r[2]*tx[2]); // e_13
}
...
/*
  ...
  actual rotation
*/
void linear_bmat( TacsScalar B[], TacsScalar rotz[],
                  int num_points,
  ...
  for ( int i = 0; i < num_points; i++ ){
    ...
    for ( int ii = 0; ii < 6; ii++ ){
      ...
      B[2] = (Ud[0]*tx[3] + Ud[1]*tx[4] + r[0]*tx[5] +
              Ud[2]*tx[0] + Ud[3]*tx[1] + r[1]*tx[2]); // e_12

      // Compute the bending components of the strain
      B[3] = (r_xi[0]*tx[0] + r_eta[0]*tx[1] +
              Ud[0]*ztx[0] + Ud[1]*ztx[1] + r[0]*ztx[2]);  // k_11
      B[4] = (r_xi[1]*tx[3] + r_eta[1]*tx[4] +
              Ud[2]*ztx[3] + Ud[3]*ztx[4] + r[1]*ztx[5]);  // k_22

      B[5] = (r_xi[0]*tx[3] + r_eta[0]*tx[4] +
              r_xi[1]*tx[0] + r_eta[1]*tx[1] +
              Ud[0]*ztx[3] + Ud[1]*ztx[4] + r[0]*ztx[5] +
              Ud[2]*ztx[0] + Ud[3]*ztx[1] + r[1]*ztx[2]); // k_12

      // Compute the shear terms
      B[6] = (Ud[2]*tx[6] + Ud[3]*tx[7] + r[1]*tx[8] +
              Ud[4]*tx[3] + Ud[5]*tx[4] + r[2]*tx[5]); // e_23
      B[7] = (Ud[0]*tx[6] + Ud[1]*tx[7] + r[0]*tx[8] +
              Ud[4]*tx[0] + Ud[5]*tx[1] + r[2]*tx[2]); // e_13
...
*/
void add_linear_bmat_sens( TacsScalar res[], int num_points,
  ...
  for ( int k = 0; k < num_components; k++ ){
    ...
    for ( int i = 0; i < num_points; i++ ){
      ...
      for ( int ii = 0; ii < 6; ii++ ){
        ...
        normal_rot_bmat(dr, dr_xi, dr_eta,
                        ii, N[i], Na[i], Nb[i],
                        dn, dn_xi, dn_eta);
        ...
        res[0] += stress_scale*(stress[0]*sux  + stress[1]*svy  +
                                stress[2]*(suy + svx) +
                                stress[3]*sux1 + stress[4]*svy1 +
                                stress[5]*(suy1 + svx1) +
                                stress[6]*(svz + swy) +
                                stress[7]*(suz + swx));
...
/*
  ...
  rotz: the difference between the drilling rotation and actual rotation
*/
void nonlinear_strain( TacsScalar strain[],
                       TacsScalar *rotz,
                       const TacsScalar Ux[],
                       const TacsScalar Uxd[],
                       const TacsScalar t[],
...
/*
  ...
  rotz: the difference between the drilling rotation and actual rotation
*/
void nonlinear_bmat( TacsScalar B[], TacsScalar rotz[],
                     int num_points,
                     const double N[], const double Na[], const double Nb[],
                     const TacsScalar Ux[],
                     const TacsScalar Uxd[],
  ...
  for ( int i = 0; i < num_points; i++ ){
    ...
    for ( int ii = 0; ii < 6; ii++ ){
      ...
      B[2] = duy + dvx + (dux*uy + dvx*vy + dwx*wy +
                          ux*duy + vx*dvy + wx*dwy);

      // Compute the bending components of the strain
      B[3] = dux1 + (dux*ux1 + dvx*vx1 + dwx*wx1 +
                     ux*dux1 + vx*dvx1 + wx*dwx1);
      B[4] = dvy1 + (duy*uy1 + dvy*vy1 + dwy*wy1 +
                     uy*duy1 + vy*dvy1 + wy*dwy1);
...
void nonlinear_stress_bmat( TacsScalar matrix[], int num_points,
  ...
  for ( int i = 0; i < num_points; i++ ){
    for ( int ii = 0; ii < 6; ii++ ){
      ...
      // The first-derivative values of the displacements
      // u1x, u1y
      du1[0] = (r_xi[0]*tx[0] + r_eta[0]*tx[1] +
                Ud[0]*ztx[0] + Ud[1]*ztx[1] + r[0]*ztx[2]);
      du1[1] = (r_xi[0]*tx[3] + r_eta[0]*tx[4] +
                Ud[0]*ztx[3] + Ud[1]*ztx[4] + r[0]*ztx[5]);

      // v1x, v1y
      du1[2] = (r_xi[1]*tx[0] + r_eta[1]*tx[1] +
                Ud[2]*ztx[0] + Ud[3]*ztx[1] + r[1]*ztx[2]);
      du1[3] = (r_xi[1]*tx[3] + r_eta[1]*tx[4] +
                Ud[2]*ztx[3] + Ud[3]*ztx[4] + r[1]*ztx[5]);

      // w1x, w1y
      du1[4] = (r_xi[2]*tx[0] + r_eta[2]*tx[1] +
                Ud[4]*ztx[0] + Ud[5]*ztx[1] + r[2]*ztx[2]);
      du1[5] = (r_xi[2]*tx[3] + r_eta[2]*tx[4] +
                Ud[4]*ztx[3] + Ud[5]*ztx[4] + r[2]*ztx[5]);
      du1 += 6;
    }
  }
...
void nonlinear_strain_sens( TacsScalar strain[], TacsScalar dstrain[],
  ...
  *rotz = (0.5*(vx - uy)
  ...
  for ( int k = 0; k < num_components; k++ ){
    ...
    dstrain[2] = suy + svx + (sux*uy + svx*vy + swx*wy +
                              ux*suy + vx*svy + wx*swy);
    ...
    dstrain[7] = suz + swx + (suz*ux + svz*vx + swz*wx +
                              uz*sux + vz*svx + wz*swx);
...
*/
void add_nonlinear_bmat_sens( TacsScalar res[], int num_points,
                              const TacsScalar stress_scale,
                              const TacsScalar rot_scale,
                              const TacsScalar stress[],
                              const double N[], const double Na[],
...
*/
void nonlinear_stress_bmat_sens( TacsScalar matrix[], int num_points,
                                 TacsScalar scale, TacsScalar sscale,
                                 const TacsScalar stress[],
    ...
    for ( int ii = 0; ii < 6; ii++ ){
      ...
      for ( int j = 0; j <= i; j++ ){
        ...
        for ( int jj = 0; jj <= end; jj++ ){
          ...
          B[2] = (dux*uy + dvx*vy + dwx*wy +
                  ux*duy + vx*dvy + wx*dwy);

          // Compute the bending components of the strain
          B[3] = (dux*ux1 + dvx*vx1 + dwx*wx1 +
                  ux*dux1 + vx*dvx1 + wx*dwx1);
          B[4] = (duy*uy1 + dvy*vy1 + dwy*wy1 +
                  uy*duy1 + vy*dvy1 + wy*dwy1);
          B[5] = (dux1*uy + dvx1*vy + dwx1*wy +
                  dux*uy1 + dvx*vy1 + dwx*wy1 +
                  ux1*duy + vx1*dvy + wx1*dwy +
                  ux*duy1 + vx*dvy1 + wx*dwy1);

          // Compute the shear terms
          B[6] = (duz*uy + dvz*vy + dwz*wy +
                  uz*duy + vz*dvy + wz*dwy);
          B[7] = (duz*ux + dvz*vx + dwz*wx +
                  uz*dux + vz*dvx + wz*dwx);

          TacsScalar bs = (B[0]*stress[0] + B[1]*stress[1] +
                           B[2]*stress[2] + B[3]*stress[3] +
                           B[4]*stress[4] + B[5]*stress[5] +
                           B[6]*stress[6] + B[7]*stress[7]);

          TacsScalar ssb = (B[0]*sstress[0] + B[1]*sstress[1] +
                            B[2]*sstress[2] + B[3]*sstress[3] +
                            B[4]*sstress[4] + B[5]*sstress[5] +
                            B[6]*sstress[6] + B[7]*sstress[7]);

          // Compute the sensitivity terms ...

          // The in plane strains
          B[0] = (sux*dux + svx*dvx + swx*dwx +
                  ux*sdux + vx*sdvx + wx*sdwx);
          B[1] = (suy*duy + svy*dvy + swy*dwy +
                  uy*sduy + vy*sdvy + wy*sdwy);
          B[2] = (sdux*uy + sdvx*vy + sdwx*wy +
                  sux*duy + svx*dvy + swx*dwy +
                  dux*suy + dvx*svy + dwx*swy +
                  ux*sduy + vx*sdvy + wx*sdwy);

          // Compute the bending components of the strain
          B[3] = (sdux*ux1 + sdvx*vx1 + sdwx*wx1 +
                  sux*dux1 + svx*dvx1 + swx*dwx1 +
                  dux*sux1 + dvx*svx1 + dwx*swx1 +
                  ux*sdux1 + vx*sdvx1 + wx*sdwx1);
          B[4] = (sduy*uy1 + sdvy*vy1 + sdwy*wy1 +
                  suy*duy1 + svy*dvy1 + swy*dwy1 +
                  duy*suy1 + dvy*svy1 + dwy*swy1 +
                  uy*sduy1 + vy*sdvy1 + wy*sdwy1);
          B[5] = (sdux1*uy + sdvx1*vy + sdwx1*wy +
                  sdux*uy1 + sdvx*vy1 + sdwx*wy1 +
                  sux1*duy + svx1*dvy + swx1*dwy +
                  sux*duy1 + svx*dvy1 + swx*dwy1 +
                  dux1*suy + dvx1*svy + dwx1*swy +
          ...
          // Compute the shear terms
          B[6] = (sduz*uy + sdvz*vy + sdwz*wy +
                  suz*duy + svz*dvy + swz*dwy +
                  duz*suy + dvz*svy + dwz*swy +
                  uz*sduy + vz*sdvy + wz*sdwy);
          B[7] = (sduz*ux + sdvz*vx + sdwz*wx +
                  suz*dux + svz*dvx + swz*dwx +
                  duz*sux + dvz*svx + dwz*swx +
                  uz*sdux + vz*sdvx + wz*sdwx);

          TacsScalar sbs = (B[0]*stress[0] + B[1]*stress[1] +
                            B[2]*stress[2] + B[3]*stress[3] +
                            B[4]*stress[4] + B[5]*stress[5] +
                            B[6]*stress[6] + B[7]*stress[7]);
...
*/
void linear_bend_strain( TacsScalar strain[],
                         TacsScalar *rotz,
                         const TacsScalar Ux[],
                         const TacsScalar Uxd[],
                         const TacsScalar t[],
...
void linear_bend_bmat( TacsScalar B[], TacsScalar rotz[],
                       int num_points,
                       const double N[], const double Na[],
                       const double Nb[],
                       const TacsScalar t[],
...
/*
  ...
  rotation
*/
void nonlinear_bend_strain( TacsScalar strain[],
                            TacsScalar *rotz,
                            const TacsScalar Ux[],
                            const TacsScalar Uxd[],
                            const TacsScalar t[],
...
*/
void nonlinear_bend_bmat( TacsScalar B[], TacsScalar rotz[],
                          int num_points,
                          const double N[], const double Na[],
                          const double Nb[],
                          const TacsScalar Ux[],
...
void nonlinear_bend_stress_bmat( TacsScalar matrix[], int num_points,
  ...
  for ( int i = 0; i < num_points; i++ ){
    for ( int ii = 0; ii < 6; ii++ ){
      ...
      TacsScalar r[3], r_xi[3], r_eta[3];
      transform_displ_gradient_bmat(Ud, ii, t, Na[i], Nb[i]);
      normal_rot_bmat(r, r_xi, r_eta, ii, N[i], Na[i], Nb[i],
                      n, n_xi, n_eta);
...
*/
void inner_nonlinear_bend_bmat( TacsScalar bstrain[],
                                const TacsScalar Uxpsi[],
                                const TacsScalar Uxdpsi[],
                                const TacsScalar Uxphi[],
                                const TacsScalar Uxdphi[],
                                const TacsScalar t[],
                                const TacsScalar tx[],
                                const TacsScalar ztx[],
                                const TacsScalar n[],
  ...
  // The first-derivative values of the displacements
  // u1x, u1y
  du1psi[0] = (r_xi[0]*tx[0] + r_eta[0]*tx[1] +
               Ud[0]*ztx[0] + Ud[1]*ztx[1] + r[0]*ztx[2]);
  du1psi[1] = (r_xi[0]*tx[3] + r_eta[0]*tx[4] +
               Ud[0]*ztx[3] + Ud[1]*ztx[4] + r[0]*ztx[5]);

  // v1x, v1y
  du1psi[2] = (r_xi[1]*tx[0] + r_eta[1]*tx[1] +
               Ud[2]*ztx[0] + Ud[3]*ztx[1] + r[1]*ztx[2]);
  du1psi[3] = (r_xi[1]*tx[3] + r_eta[1]*tx[4] +
               Ud[2]*ztx[3] + Ud[3]*ztx[4] + r[1]*ztx[5]);

  // w1x, w1y
  du1psi[4] = (r_xi[2]*tx[0] + r_eta[2]*tx[1] +
               Ud[4]*ztx[0] + Ud[5]*ztx[1] + r[2]*ztx[2]);
  du1psi[5] = (r_xi[2]*tx[3] + r_eta[2]*tx[4] +
               Ud[4]*ztx[3] + Ud[5]*ztx[4] + r[2]*ztx[5]);
  ...
  // The first-derivative values of the displacements
  // u1x, u1y
  du1phi[0] = (r_xi[0]*tx[0] + r_eta[0]*tx[1] +
               Ud[0]*ztx[0] + Ud[1]*ztx[1] + r[0]*ztx[2]);
  du1phi[1] = (r_xi[0]*tx[3] + r_eta[0]*tx[4] +
               Ud[0]*ztx[3] + Ud[1]*ztx[4] + r[0]*ztx[5]);

  // v1x, v1y
  du1phi[2] = (r_xi[1]*tx[0] + r_eta[1]*tx[1] +
               Ud[2]*ztx[0] + Ud[3]*ztx[1] + r[1]*ztx[2]);
  du1phi[3] = (r_xi[1]*tx[3] + r_eta[1]*tx[4] +
               Ud[2]*ztx[3] + Ud[3]*ztx[4] + r[1]*ztx[5]);

  // w1x, w1y
  du1phi[4] = (r_xi[2]*tx[0] + r_eta[2]*tx[1] +
               Ud[4]*ztx[0] + Ud[5]*ztx[1] + r[2]*ztx[2]);
  du1phi[5] = (r_xi[2]*tx[3] + r_eta[2]*tx[4] +
               Ud[4]*ztx[3] + Ud[5]*ztx[4] + r[2]*ztx[5]);

  // Compute the bending components of the strain
  bstrain[3] = (du0phi[0]*du1psi[0] + du0phi[2]*du1psi[2] + du0phi[4]*du1psi[4] +
                du1phi[0]*du0psi[0] + du1phi[2]*du0psi[2] + du1phi[4]*du0psi[4]);
  bstrain[4] = (du0phi[1]*du1psi[1] + du0phi[3]*du1psi[3] + du0phi[5]*du1psi[5] +
                du1phi[1]*du0psi[1] + du1phi[3]*du0psi[3] + du1phi[5]*du0psi[5]);
  bstrain[5] = (du0phi[0]*du1psi[1] + du0phi[2]*du1psi[3] + du0phi[4]*du1psi[5] +
...
/*
  ...
  drotz: the sensitivity of rotz
*/
void nonlinear_bend_strain_sens( TacsScalar strain[], TacsScalar dstrain[],
                                 TacsScalar *rotz, TacsScalar *drotz,
                                 const TacsScalar Ux[],
                                 const TacsScalar Uxd[],
                                 const TacsScalar t[], const TacsScalar dt[],
  ...
  for ( int k = 0; k < num_components; k++ ){
    ...
    for ( int i = 0; i < num_points; i++ ){
      ...
      for ( int ii = 0; ii < 6; ii++ ){
        ...
        // Compute the bending components of the strain
        dB[0] = sdux1 + (sdux*ux1 + sdvx*vx1 + sdwx*wx1 +
                         sux*dux1 + svx*dvx1 + swx*dwx1 +
                         dux*sux1 + dvx*svx1 + dwx*swx1 +
                         ux*sdux1 + vx*sdvx1 + wx*sdwx1);
        dB[1] = sdvy1 + (sduy*uy1 + sdvy*vy1 + sdwy*wy1 +
                         suy*duy1 + svy*dvy1 + swy*dwy1 +
                         duy*suy1 + dvy*svy1 + dwy*swy1 +
                         uy*sduy1 + vy*sdvy1 + wy*sdwy1);
        dB[2] = sduy1 + sdvx1 + (sdux1*uy + sdvx1*vy + sdwx1*wy +
                                 sdux*uy1 + sdvx*vy1 + sdwx*wy1 +
                                 sux1*duy + svx1*dvy + swx1*dwy +
                                 sux*duy1 + svx*dvy1 + swx*dwy1 +
                                 dux1*suy + dvx1*svy + dwx1*swy +
...
/*
  ...
  displacements.
*/
void compute_tensorial_components( const int order,
  ...
  if (order == 2){
    ...
    Urot[0] = vars[3]*N[0] + vars[9]*N[1]  + vars[15]*N[2] + vars[21]*N[3];
    Urot[1] = vars[4]*N[0] + vars[10]*N[1] + vars[16]*N[2] + vars[22]*N[3];
    Urot[2] = vars[5]*N[0] + vars[11]*N[1] + vars[17]*N[2] + vars[23]*N[3];

    // Compute the derivatives of the displacement
    Ud[0]  = vars[0]*Na[0] + vars[6]*Na[1]  + vars[12]*Na[2] + vars[18]*Na[3];
    Ud[2]  = vars[1]*Na[0] + vars[7]*Na[1]  + vars[13]*Na[2] + vars[19]*Na[3];
    Ud[4]  = vars[2]*Na[0] + vars[8]*Na[1]  + vars[14]*Na[2] + vars[20]*Na[3];

    // Compute the derivative of X along a
    Xd[0] = Xpts[0]*Na[0] + Xpts[3]*Na[1] + Xpts[6]*Na[2] + Xpts[9]*Na[3];
    Xd[1] = Xpts[1]*Na[0] + Xpts[4]*Na[1] + Xpts[7]*Na[2] + Xpts[10]*Na[3];
    Xd[2] = Xpts[2]*Na[0] + Xpts[5]*Na[1] + Xpts[8]*Na[2] + Xpts[11]*Na[3];

    // Compute the derivative of the displacement
    Ud[1]  = vars[0]*Nb[0] + vars[6]*Nb[1]  + vars[12]*Nb[2] + vars[18]*Nb[3];
    Ud[3]  = vars[1]*Nb[0] + vars[7]*Nb[1]  + vars[13]*Nb[2] + vars[19]*Nb[3];
    Ud[5]  = vars[2]*Nb[0] + vars[8]*Nb[1]  + vars[14]*Nb[2] + vars[20]*Nb[3];

    // Compute the derivative of X along b
    Xd[3] = Xpts[0]*Nb[0] + Xpts[3]*Nb[1] + Xpts[6]*Nb[2] + Xpts[9]*Nb[3];
    Xd[4] = Xpts[1]*Nb[0] + Xpts[4]*Nb[1] + Xpts[7]*Nb[2] + Xpts[10]*Nb[3];
    Xd[5] = Xpts[2]*Nb[0] + Xpts[5]*Nb[1] + Xpts[8]*Nb[2] + Xpts[11]*Nb[3];
  }
  else if (order == 3){
    FElibrary::biLagrangeSF(N, Na, Nb, pt, 3);

    // Compute the rotations
    Urot[0] = (vars[3]*N[0] + vars[9]*N[1]  + vars[15]*N[2] + vars[21]*N[3] + vars[27]*N[4] + vars[33]*N[5] + vars[39]*N[6] + vars[45]*N[7] + vars[51]*N[8]);
    Urot[1] = (vars[4]*N[0] + vars[10]*N[1] + vars[16]*N[2] + vars[22]*N[3] + vars[28]*N[4] + vars[34]*N[5] + vars[40]*N[6] + vars[46]*N[7] + vars[52]*N[8]);
    Urot[2] = (vars[5]*N[0] + vars[11]*N[1] + vars[17]*N[2] + vars[23]*N[3] + vars[29]*N[4] + vars[35]*N[5] + vars[41]*N[6] + vars[47]*N[7] + vars[53]*N[8]);

    // Compute the derivatives of U along the a-direction
    Ud[0] = (vars[0]*Na[0] + vars[6]*Na[1]  + vars[12]*Na[2] + vars[18]*Na[3] + vars[24]*Na[4] + vars[30]*Na[5] + vars[36]*Na[6] + vars[42]*Na[7] + vars[48]*Na[8]);
    Ud[2] = (vars[1]*Na[0] + vars[7]*Na[1]  + vars[13]*Na[2] + vars[19]*Na[3] + vars[25]*Na[4] + vars[31]*Na[5] + vars[37]*Na[6] + vars[43]*Na[7] + vars[49]*Na[8]);
    Ud[4] = (vars[2]*Na[0] + vars[8]*Na[1]  + vars[14]*Na[2] + vars[20]*Na[3] + vars[26]*Na[4] + vars[32]*Na[5] + vars[38]*Na[6] + vars[44]*Na[7] + vars[50]*Na[8]);

    // Compute the derivative of X along a
    Xd[0] = Xpts[0]*Na[0] + Xpts[3]*Na[1] + Xpts[6]*Na[2] + Xpts[9]*Na[3]  + Xpts[12]*Na[4] + Xpts[15]*Na[5] + Xpts[18]*Na[6] + Xpts[21]*Na[7] + Xpts[24]*Na[8];
    Xd[1] = Xpts[1]*Na[0] + Xpts[4]*Na[1] + Xpts[7]*Na[2] + Xpts[10]*Na[3] + Xpts[13]*Na[4] + Xpts[16]*Na[5] + Xpts[19]*Na[6] + Xpts[22]*Na[7] + Xpts[25]*Na[8];
    Xd[2] = Xpts[2]*Na[0] + Xpts[5]*Na[1] + Xpts[8]*Na[2] + Xpts[11]*Na[3] + Xpts[14]*Na[4] + Xpts[17]*Na[5] + Xpts[20]*Na[6] + Xpts[23]*Na[7] + Xpts[26]*Na[8];

    // Compute the derivatives along the b-direction
    Ud[1] = (vars[0]*Nb[0] + vars[6]*Nb[1]  + vars[12]*Nb[2] + vars[18]*Nb[3] + vars[24]*Nb[4] + vars[30]*Nb[5] + vars[36]*Nb[6] + vars[42]*Nb[7] + vars[48]*Nb[8]);
    Ud[3] = (vars[1]*Nb[0] + vars[7]*Nb[1]  + vars[13]*Nb[2] + vars[19]*Nb[3] + vars[25]*Nb[4] + vars[31]*Nb[5] + vars[37]*Nb[6] + vars[43]*Nb[7] + vars[49]*Nb[8]);
    Ud[5] = (vars[2]*Nb[0] + vars[8]*Nb[1]  + vars[14]*Nb[2] + vars[20]*Nb[3] + vars[26]*Nb[4] + vars[32]*Nb[5] + vars[38]*Nb[6] + vars[44]*Nb[7] + vars[50]*Nb[8]);

    // Compute the derivative of X along a
    Xd[3] = Xpts[0]*Nb[0] + Xpts[3]*Nb[1] + Xpts[6]*Nb[2] + Xpts[9]*Nb[3]  + Xpts[12]*Nb[4] + Xpts[15]*Nb[5] + Xpts[18]*Nb[6] + Xpts[21]*Nb[7] + Xpts[24]*Nb[8];
    Xd[4] = Xpts[1]*Nb[0] + Xpts[4]*Nb[1] + Xpts[7]*Nb[2] + Xpts[10]*Nb[3] + Xpts[13]*Nb[4] + Xpts[16]*Nb[5] + Xpts[19]*Nb[6] + Xpts[22]*Nb[7] + Xpts[25]*Nb[8];
    Xd[5] = Xpts[2]*Nb[0] + Xpts[5]*Nb[1] + Xpts[8]*Nb[2] + Xpts[11]*Nb[3] + Xpts[14]*Nb[4] + Xpts[17]*Nb[5] + Xpts[20]*Nb[6] + Xpts[23]*Nb[7] + Xpts[26]*Nb[8];
  }
  else if (order == 4){
    FElibrary::biLagrangeSF(N, Na, Nb, pt, 4);

    // Compute the shape functions and rotations
    Urot[0] = (vars[3]*N[0]  + vars[9]*N[1]  + vars[15]*N[2]  + vars[21]*N[3]  + vars[27]*N[4]  + vars[33]*N[5]  + vars[39]*N[6]  + vars[45]*N[7] +
               vars[51]*N[8] + vars[57]*N[9] + vars[63]*N[10] + vars[69]*N[11] + vars[75]*N[12] + vars[81]*N[13] + vars[87]*N[14] + vars[93]*N[15]);
    Urot[1] = (vars[4]*N[0]  + vars[10]*N[1] + vars[16]*N[2]  + vars[22]*N[3]  + vars[28]*N[4]  + vars[34]*N[5]  + vars[40]*N[6]  + vars[46]*N[7] +
               vars[52]*N[8] + vars[58]*N[9] + vars[64]*N[10] + vars[70]*N[11] + vars[76]*N[12] + vars[82]*N[13] + vars[88]*N[14] + vars[94]*N[15]);
    Urot[2] = (vars[5]*N[0]  + vars[11]*N[1] + vars[17]*N[2]  + vars[23]*N[3]  + vars[29]*N[4]  + vars[35]*N[5]  + vars[41]*N[6]  + vars[47]*N[7] +
    ...
    // derivatives along a
    Ud[0] = (vars[0]*Na[0]  + vars[6]*Na[1]  + vars[12]*Na[2]  + vars[18]*Na[3]  + vars[24]*Na[4]  + vars[30]*Na[5]  + vars[36]*Na[6]  + vars[42]*Na[7] +
             vars[48]*Na[8] + vars[54]*Na[9] + vars[60]*Na[10] + vars[66]*Na[11] + vars[72]*Na[12] + vars[78]*Na[13] + vars[84]*Na[14] + vars[90]*Na[15]);
    Ud[2] = (vars[1]*Na[0]  + vars[7]*Na[1]  + vars[13]*Na[2]  + vars[19]*Na[3]  + vars[25]*Na[4]  + vars[31]*Na[5]  + vars[37]*Na[6]  + vars[43]*Na[7] +
             vars[49]*Na[8] + vars[55]*Na[9] + vars[61]*Na[10] + vars[67]*Na[11] + vars[73]*Na[12] + vars[79]*Na[13] + vars[85]*Na[14] + vars[91]*Na[15]);
    Ud[4] = (vars[2]*Na[0]  + vars[8]*Na[1]  + vars[14]*Na[2]  + vars[20]*Na[3]  + vars[26]*Na[4]  + vars[32]*Na[5]  + vars[38]*Na[6]  + vars[44]*Na[7] +
             vars[50]*Na[8] + vars[56]*Na[9] + vars[62]*Na[10] + vars[68]*Na[11] + vars[74]*Na[12] + vars[80]*Na[13] + vars[86]*Na[14] + vars[92]*Na[15]);

    Xd[0] = (Xpts[0]*Na[0]  + Xpts[3]*Na[1]  + Xpts[6]*Na[2]   + Xpts[9]*Na[3]   + Xpts[12]*Na[4]  + Xpts[15]*Na[5]  + Xpts[18]*Na[6]  + Xpts[21]*Na[7] +
             Xpts[24]*Na[8] + Xpts[27]*Na[9] + Xpts[30]*Na[10] + Xpts[33]*Na[11] + Xpts[36]*Na[12] + Xpts[39]*Na[13] + Xpts[42]*Na[14] + Xpts[45]*Na[15]);
    Xd[1] = (Xpts[1]*Na[0]  + Xpts[4]*Na[1]  + Xpts[7]*Na[2]   + Xpts[10]*Na[3]  + Xpts[13]*Na[4]  + Xpts[16]*Na[5]  + Xpts[19]*Na[6]  + Xpts[22]*Na[7] +
             Xpts[25]*Na[8] + Xpts[28]*Na[9] + Xpts[31]*Na[10] + Xpts[34]*Na[11] + Xpts[37]*Na[12] + Xpts[40]*Na[13] + Xpts[43]*Na[14] + Xpts[46]*Na[15]);
    Xd[2] = (Xpts[2]*Na[0]  + Xpts[5]*Na[1]  + Xpts[8]*Na[2]   + Xpts[11]*Na[3]  + Xpts[14]*Na[4]  + Xpts[17]*Na[5]  + Xpts[20]*Na[6]  + Xpts[23]*Na[7] +
             Xpts[26]*Na[8] + Xpts[29]*Na[9] + Xpts[32]*Na[10] + Xpts[35]*Na[11] + Xpts[38]*Na[12] + Xpts[41]*Na[13] + Xpts[44]*Na[14] + Xpts[47]*Na[15]);

    // Compute the derivatives of the shape functions along the
    // parametric b-direction
    Ud[1] = (vars[0]*Nb[0]  + vars[6]*Nb[1]  + vars[12]*Nb[2]  + vars[18]*Nb[3]  + vars[24]*Nb[4]  + vars[30]*Nb[5]  + vars[36]*Nb[6]  + vars[42]*Nb[7] +
             vars[48]*Nb[8] + vars[54]*Nb[9] + vars[60]*Nb[10] + vars[66]*Nb[11] + vars[72]*Nb[12] + vars[78]*Nb[13] + vars[84]*Nb[14] + vars[90]*Nb[15]);
    Ud[3] = (vars[1]*Nb[0]  + vars[7]*Nb[1]  + vars[13]*Nb[2]  + vars[19]*Nb[3]  + vars[25]*Nb[4]  + vars[31]*Nb[5]  + vars[37]*Nb[6]  + vars[43]*Nb[7] +
             vars[49]*Nb[8] + vars[55]*Nb[9] + vars[61]*Nb[10] + vars[67]*Nb[11] + vars[73]*Nb[12] + vars[79]*Nb[13] + vars[85]*Nb[14] + vars[91]*Nb[15]);
    Ud[5] = (vars[2]*Nb[0]  + vars[8]*Nb[1]  + vars[14]*Nb[2]  + vars[20]*Nb[3]  + vars[26]*Nb[4]  + vars[32]*Nb[5]  + vars[38]*Nb[6]  + vars[44]*Nb[7] +
             vars[50]*Nb[8] + vars[56]*Nb[9] + vars[62]*Nb[10] + vars[68]*Nb[11] + vars[74]*Nb[12] + vars[80]*Nb[13] + vars[86]*Nb[14] + vars[92]*Nb[15]);

    Xd[3] = (Xpts[0]*Nb[0]  + Xpts[3]*Nb[1]  + Xpts[6]*Nb[2]   + Xpts[9]*Nb[3]   + Xpts[12]*Nb[4]  + Xpts[15]*Nb[5]  + Xpts[18]*Nb[6]  + Xpts[21]*Nb[7] +
             Xpts[24]*Nb[8] + Xpts[27]*Nb[9] + Xpts[30]*Nb[10] + Xpts[33]*Nb[11] + Xpts[36]*Nb[12] + Xpts[39]*Nb[13] + Xpts[42]*Nb[14] + Xpts[45]*Nb[15]);
    Xd[4] = (Xpts[1]*Nb[0]  + Xpts[4]*Nb[1]  + Xpts[7]*Nb[2]   + Xpts[10]*Nb[3]  + Xpts[13]*Nb[4]  + Xpts[16]*Nb[5]  + Xpts[19]*Nb[6]  + Xpts[22]*Nb[7] +
             Xpts[25]*Nb[8] + Xpts[28]*Nb[9] + Xpts[31]*Nb[10] + Xpts[34]*Nb[11] + Xpts[37]*Nb[12] + Xpts[40]*Nb[13] + Xpts[43]*Nb[14] + Xpts[46]*Nb[15]);
    Xd[5] = (Xpts[2]*Nb[0]  + Xpts[5]*Nb[1]  + Xpts[8]*Nb[2]   + Xpts[11]*Nb[3]  + Xpts[14]*Nb[4]  + Xpts[17]*Nb[5]  + Xpts[20]*Nb[6]  + Xpts[23]*Nb[7] +
             Xpts[26]*Nb[8] + Xpts[29]*Nb[9] + Xpts[32]*Nb[10] + Xpts[35]*Nb[11] + Xpts[38]*Nb[12] + Xpts[41]*Nb[13] + Xpts[44]*Nb[14] + Xpts[47]*Nb[15]);
  }
  else {
...
*/
void compute_shell_Ud( const int num_nodes,
  ...
  if (num_nodes == 4){
    ...
    Ud[2]  = vars[1]*Na[0] + vars[7]*Na[1]  + vars[13]*Na[2] + vars[19]*Na[3];
    Ud[4]  = vars[2]*Na[0] + vars[8]*Na[1]  + vars[14]*Na[2] + vars[20]*Na[3];
    Ud[6]  = vars[3]*Na[0] + vars[9]*Na[1]  + vars[15]*Na[2] + vars[21]*Na[3];
    Ud[8]  = vars[4]*Na[0] + vars[10]*Na[1] + vars[16]*Na[2] + vars[22]*Na[3];
    Ud[10] = vars[5]*Na[0] + vars[11]*Na[1] + vars[17]*Na[2] + vars[23]*Na[3];

    Ud[1]  = vars[0]*Nb[0] + vars[6]*Nb[1]  + vars[12]*Nb[2] + vars[18]*Nb[3];
    Ud[3]  = vars[1]*Nb[0] + vars[7]*Nb[1]  + vars[13]*Nb[2] + vars[19]*Nb[3];
    Ud[5]  = vars[2]*Nb[0] + vars[8]*Nb[1]  + vars[14]*Nb[2] + vars[20]*Nb[3];
    Ud[7]  = vars[3]*Nb[0] + vars[9]*Nb[1]  + vars[15]*Nb[2] + vars[21]*Nb[3];
    Ud[9]  = vars[4]*Nb[0] + vars[10]*Nb[1] + vars[16]*Nb[2] + vars[22]*Nb[3];
    Ud[11] = vars[5]*Nb[0] + vars[11]*Nb[1] + vars[17]*Nb[2] + vars[23]*Nb[3];
  }
  else if (num_nodes == 9){
    U[0] = (vars[0]*N[0] + vars[6]*N[1]  + vars[12]*N[2] + vars[18]*N[3] + vars[24]*N[4] + vars[30]*N[5] + vars[36]*N[6] + vars[42]*N[7] + vars[48]*N[8]);
    U[1] = (vars[1]*N[0] + vars[7]*N[1]  + vars[13]*N[2] + vars[19]*N[3] + vars[25]*N[4] + vars[31]*N[5] + vars[37]*N[6] + vars[43]*N[7] + vars[49]*N[8]);
    ...
    Ud[2] = (vars[1]*Na[0] + vars[7]*Na[1]  + vars[13]*Na[2] + vars[19]*Na[3] + vars[25]*Na[4] + vars[31]*Na[5] + vars[37]*Na[6] + vars[43]*Na[7] + vars[49]*Na[8]);
    Ud[4] = (vars[2]*Na[0] + vars[8]*Na[1]  + vars[14]*Na[2] + vars[20]*Na[3] + vars[26]*Na[4] + vars[32]*Na[5] + vars[38]*Na[6] + vars[44]*Na[7] + vars[50]*Na[8]);
    Ud[6] = (vars[3]*Na[0] + vars[9]*Na[1]  + vars[15]*Na[2] + vars[21]*Na[3] + vars[27]*Na[4] + vars[33]*Na[5] + vars[39]*Na[6] + vars[45]*Na[7] + vars[51]*Na[8]);
    Ud[8] = (vars[4]*Na[0] + vars[10]*Na[1] + vars[16]*Na[2] + vars[22]*Na[3] + vars[28]*Na[4] + vars[34]*Na[5] + vars[40]*Na[6] + vars[46]*Na[7] + vars[52]*Na[8]);
    Ud[10] = (vars[5]*Na[0] + vars[11]*Na[1] + vars[17]*Na[2] + vars[23]*Na[3] + vars[29]*Na[4] + vars[35]*Na[5] + vars[41]*Na[6] + vars[47]*Na[7] + vars[53]*Na[8]);

    Ud[1] = (vars[0]*Nb[0] + vars[6]*Nb[1]  + vars[12]*Nb[2] + vars[18]*Nb[3] + vars[24]*Nb[4] + vars[30]*Nb[5] + vars[36]*Nb[6] + vars[42]*Nb[7] + vars[48]*Nb[8]);
    Ud[3] = (vars[1]*Nb[0] + vars[7]*Nb[1]  + vars[13]*Nb[2] + vars[19]*Nb[3] + vars[25]*Nb[4] + vars[31]*Nb[5] + vars[37]*Nb[6] + vars[43]*Nb[7] + vars[49]*Nb[8]);
    Ud[5] = (vars[2]*Nb[0] + vars[8]*Nb[1]  + vars[14]*Nb[2] + vars[20]*Nb[3] + vars[26]*Nb[4] + vars[32]*Nb[5] + vars[38]*Nb[6] + vars[44]*Nb[7] + vars[50]*Nb[8]);
    Ud[7] = (vars[3]*Nb[0] + vars[9]*Nb[1]  + vars[15]*Nb[2] + vars[21]*Nb[3] + vars[27]*Nb[4] + vars[33]*Nb[5] + vars[39]*Nb[6] + vars[45]*Nb[7] + vars[51]*Nb[8]);
    Ud[9] = (vars[4]*Nb[0] + vars[10]*Nb[1] + vars[16]*Nb[2] + vars[22]*Nb[3] + vars[28]*Nb[4] + vars[34]*Nb[5] + vars[40]*Nb[6] + vars[46]*Nb[7] + vars[52]*Nb[8]);
    Ud[11] = (vars[5]*Nb[0] + vars[11]*Nb[1] + vars[17]*Nb[2] + vars[23]*Nb[3] + vars[29]*Nb[4] + vars[35]*Nb[5] + vars[41]*Nb[6] + vars[47]*Nb[7] + vars[53]*Nb[8]);
  }
  else if (num_nodes == 16){
    U[0] = (vars[0]*N[0]  + vars[6]*N[1]  + vars[12]*N[2]  + vars[18]*N[3]  + vars[24]*N[4]  + vars[30]*N[5]  + vars[36]*N[6]  + vars[42]*N[7] +
            vars[48]*N[8] + vars[54]*N[9] + vars[60]*N[10] + vars[66]*N[11] + vars[72]*N[12] + vars[78]*N[13] + vars[84]*N[14] + vars[90]*N[15]);
    ...
    Ud[4] = (vars[2]*Na[0]  + vars[8]*Na[1]  + vars[14]*Na[2]  + vars[20]*Na[3]  + vars[26]*Na[4]  + vars[32]*Na[5]  + vars[38]*Na[6]  + vars[44]*Na[7] +
             vars[50]*Na[8] + vars[56]*Na[9] + vars[62]*Na[10] + vars[68]*Na[11] + vars[74]*Na[12] + vars[80]*Na[13] + vars[86]*Na[14] + vars[92]*Na[15]);
    Ud[6] = (vars[3]*Na[0]  + vars[9]*Na[1]  + vars[15]*Na[2]  + vars[21]*Na[3]  + vars[27]*Na[4]  + vars[33]*Na[5]  + vars[39]*Na[6]  + vars[45]*Na[7] +
             vars[51]*Na[8] + vars[57]*Na[9] + vars[63]*Na[10] + vars[69]*Na[11] + vars[75]*Na[12] + vars[81]*Na[13] + vars[87]*Na[14] + vars[93]*Na[15]);
    Ud[8] = (vars[4]*Na[0]  + vars[10]*Na[1] + vars[16]*Na[2]  + vars[22]*Na[3]  + vars[28]*Na[4]  + vars[34]*Na[5]  + vars[40]*Na[6]  + vars[46]*Na[7] +
             vars[52]*Na[8] + vars[58]*Na[9] + vars[64]*Na[10] + vars[70]*Na[11] + vars[76]*Na[12] + vars[82]*Na[13] + vars[88]*Na[14] + vars[94]*Na[15]);
    Ud[10]= (vars[5]*Na[0]  + vars[11]*Na[1] + vars[17]*Na[2]  + vars[23]*Na[3]  + vars[29]*Na[4]  + vars[35]*Na[5]  + vars[41]*Na[6]  + vars[47]*Na[7] +
             vars[53]*Na[8] + vars[59]*Na[9] + vars[65]*Na[10] + vars[71]*Na[11] + vars[77]*Na[12] + vars[83]*Na[13] + vars[89]*Na[14] + vars[95]*Na[15]);
    ...
    Ud[5] = (vars[2]*Nb[0]  + vars[8]*Nb[1]  + vars[14]*Nb[2]  + vars[20]*Nb[3]  + vars[26]*Nb[4]  + vars[32]*Nb[5]  + vars[38]*Nb[6]  + vars[44]*Nb[7] +
             vars[50]*Nb[8] + vars[56]*Nb[9] + vars[62]*Nb[10] + vars[68]*Nb[11] + vars[74]*Nb[12] + vars[80]*Nb[13] + vars[86]*Nb[14] + vars[92]*Nb[15]);
    Ud[7] = (vars[3]*Nb[0]  + vars[9]*Nb[1]  + vars[15]*Nb[2]  + vars[21]*Nb[3]  + vars[27]*Nb[4]  + vars[33]*Nb[5]  + vars[39]*Nb[6]  + vars[45]*Nb[7] +
             vars[51]*Nb[8] + vars[57]*Nb[9] + vars[63]*Nb[10] + vars[69]*Nb[11] + vars[75]*Nb[12] + vars[81]*Nb[13] + vars[87]*Nb[14] + vars[93]*Nb[15]);
    Ud[9] = (vars[4]*Nb[0]  + vars[10]*Nb[1] + vars[16]*Nb[2]  + vars[22]*Nb[3]  + vars[28]*Nb[4]  + vars[34]*Nb[5]  + vars[40]*Nb[6]  + vars[46]*Nb[7] +
             vars[52]*Nb[8] + vars[58]*Nb[9] + vars[64]*Nb[10] + vars[70]*Nb[11] + vars[76]*Nb[12] + vars[82]*Nb[13] + vars[88]*Nb[14] + vars[94]*Nb[15]);
    Ud[11]= (vars[5]*Nb[0]  + vars[11]*Nb[1] + vars[17]*Nb[2]  + vars[23]*Nb[3]  + vars[29]*Nb[4]  + vars[35]*Nb[5]  + vars[41]*Nb[6]  + vars[47]*Nb[7] +
             vars[53]*Nb[8] + vars[59]*Nb[9] + vars[65]*Nb[10] + vars[71]*Nb[11] + vars[77]*Nb[12] + vars[83]*Nb[13] + vars[89]*Nb[14] + vars[95]*Nb[15]);
  }
  else {
    U[0] = U[1] = U[2] = U[3] = U[4] = U[5] = 0.0;
    Ud[0] = Ud[1] = Ud[2] = Ud[3] = Ud[4] = Ud[5] = 0.0;
...
*/
void bspline_tensorial_components( const int order,
                                   TacsScalar Urot[],
...
/*
  ...
  Na, Nb: the first derivatives of the shape functions
*/
void shell_jacobian( const int order,
...
/*
  ...
  Naa, Nab, Nbb: the second derivatives of the shape functions
*/
void shell_hessian( const int order,
                    TacsScalar X[], TacsScalar Xd[],
                    TacsScalar Xdd[],
                    double N[], double Na[], double Nb[],
