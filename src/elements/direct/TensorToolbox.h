...
*/
template <class ScalarType> 
inline void crossProduct3D( ScalarType out[], 
...
*/
template <class ScalarType> 
inline void crossProduct3DSens( ScalarType sout[], 
                                const ScalarType A[], const ScalarType B[], 
                                const ScalarType sA[], const ScalarType sB[] ){
  //   (A2 * B3 - A3 * B2 ), ( A3 *B1 - A1 * B3 ), ( A1 * B2 - A2 * B1 )
  sout[0] = (sA[1]*B[2] - sA[2]*B[1] + A[1]*sB[2] - A[2]*sB[1]);
...
inline void crossProduct3DSens_d( ScalarType out[], ScalarType sout[], 
                                  ...
                                  const ScalarType sAd[], const ScalarType sBd[] ){
  //   (A2 * B3 - A3 * B2 ), ( A3 *B1 - A1 * B3 ), ( A1 * B2 - A2 * B1 )    
  out[0] = A[1]*B[2] - A[2]*B[1];
  out[1] = A[2]*B[0] - A[0]*B[2];
  out[2] = A[0]*B[1] - A[1]*B[0];
...
inline ScalarType normalize3DSens( ScalarType sout[], const ScalarType A[], 
                                   const ScalarType sA[], const ScalarType Anrm ){
...
*/
template <class ScalarType> 
inline ScalarType normalize3DSens( ScalarType * sAnrm, 
                                   ScalarType A[], ScalarType sA[] ){
  ScalarType Anrm = sqrt(A[0]*A[0] + A[1]*A[1] + A[2]*A[2]);
  ScalarType invAnrm = 1.0/Anrm;
...
*/
template <class ScalarType> 
inline void normalize3DSens_d( ScalarType A[], ScalarType sA[], 
                               ScalarType Ad[], ScalarType sAd[] ){
  ScalarType Anrm;
  ScalarType Anrmd;
  ScalarType arg1;
  ScalarType arg1d;
  arg1d = Ad[0]*A[0] + A[0]*Ad[0] + Ad[1]*A[1] + 
    A[1]*Ad[1] + Ad[2]*A[2] + A[2]*Ad[2];
  ...
  A[2] = A[2]/Anrm;
  sAd[0] = (sAd[0]*Anrm-sA[0]*Anrmd)/(Anrm*Anrm) - (Ad[0]*Anrm-A[0]*Anrmd)*
    sAnrm/(Anrm*Anrm) - A[0]*sAnrmd/Anrm;
  sA[0] = sA[0]/Anrm - A[0]/Anrm*sAnrm;
  sAd[1] = (sAd[1]*Anrm-sA[1]*Anrmd)/(Anrm*Anrm) - (Ad[1]*Anrm-A[1]*Anrmd)*
    sAnrm/(Anrm*Anrm) - A[1]*sAnrmd/Anrm;
  sA[1] = sA[1]/Anrm - A[1]/Anrm*sAnrm;
  sAd[2] = (sAd[2]*Anrm-sA[2]*Anrmd)/(Anrm*Anrm) - (Ad[2]*Anrm-A[2]*Anrmd)*
    sAnrm/(Anrm*Anrm) - A[2]*sAnrmd/Anrm;
...
template <class ScalarType> 
inline void transform3DStrain( ScalarType newStrain[], 
                               const ScalarType strain[], 
...
inline void transform3DStrain( ScalarType output[], const ScalarType input[], 
  ...
  */
  ...
  as[0] = trans[0]*input[0] + 0.5*trans[1]*input[5] + 0.5*trans[2]*input[4];
  as[3] = trans[3]*input[0] + 0.5*trans[4]*input[5] + 0.5*trans[5]*input[4];
  as[6] = trans[6]*input[0] + 0.5*trans[7]*input[5] + 0.5*trans[8]*input[4];
    
  as[1] = 0.5*trans[0]*input[5] + trans[1]*input[1] + 0.5*trans[2]*input[3];
  as[4] = 0.5*trans[3]*input[5] + trans[4]*input[1] + 0.5*trans[5]*input[3];
  as[7] = 0.5*trans[6]*input[5] + trans[7]*input[1] + 0.5*trans[8]*input[3];
    
  as[2] = 0.5*trans[0]*input[4] + 0.5*trans[1]*input[3] + trans[2]*input[2];
  as[5] = 0.5*trans[3]*input[4] + 0.5*trans[4]*input[3] + trans[5]*input[2];
  as[8] = 0.5*trans[6]*input[4] + 0.5*trans[7]*input[3] + trans[8]*input[2];
