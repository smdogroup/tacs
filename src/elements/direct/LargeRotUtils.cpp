...
void test_rate_matrix( TacsScalar U[], double dh ){
  ...
  for ( int k = 0; k < 3; k++ ){
    ...
    if (k == 0){
      t_index[0] = 0;   tt_index[0] = 0;
      t_index[1] = 9;   tt_index[1] = 27;
    }
    if (k == 1){
      t_index[0] = 9;   tt_index[0] = 9;
      t_index[1] = 18;  tt_index[1] = 45;
    }
    if (k == 2){
...
TacsScalar compute_inplane_penalty( TacsScalar drot[],
  ...
  v1[0] = (C[0]+1.0)*(Xd[0]+Ud[0]) + C[3]*(Xd[1]+Ud[2]) + C[6]*(Xd[2]+Ud[4]);
  v1[1] = C[1]*(Xd[0]+Ud[0]) + (C[4]+1.0)*(Xd[1]+Ud[2]) + C[7]*(Xd[2]+Ud[4]);
  v1[2] = C[2]*(Xd[0]+Ud[0]) + C[5]*(Xd[1]+Ud[2]) + (C[8]+1.0)*(Xd[2]+Ud[4]);

  // Compute v2 = dot(C^{T} + I, Xd[3:6] + Ud[1:6:2]))
  v2[0] = (C[0]+1.0)*(Xd[3]+Ud[1]) + C[3]*(Xd[4]+Ud[3]) + C[6]*(Xd[5]+Ud[5]);
  v2[1] = C[1]*(Xd[3]+Ud[1]) + (C[4]+1.0)*(Xd[4]+Ud[3]) + C[7]*(Xd[5]+Ud[5]);
  v2[2] = C[2]*(Xd[3]+Ud[1]) + C[5]*(Xd[4]+Ud[3]) + (C[8]+1.0)*(Xd[5]+Ud[5]);
  ...
  for ( int i = 0; i < num_points; i++ ){
    ...
    for ( int j = 0; j < 3; j++ ){
      ...
      dv2[2] = Cd[2]*(Xd[3]+Ud[1]) + Cd[5]*(Xd[4]+Ud[3]) + Cd[8]*(Xd[5]+Ud[5]);
...
void add_inplane_penalty( TacsScalar matrix[], const int num_points,
  ...
  for ( int i = 0; i < 6; i++ ){
    ...
    // Compute v2 = dot(C^{T} + I, Xd[3:6] + Ud[1:6:2]))
    ddv2[3*i] =     C[0]*(Xd[3]+Ud[1]) + C[3]*(Xd[4]+Ud[3]) + C[6]*(Xd[5]+Ud[5]);
    ddv2[1 + 3*i] = C[1]*(Xd[3]+Ud[1]) + C[4]*(Xd[4]+Ud[3]) + C[7]*(Xd[5]+Ud[5]);
    ddv2[2 + 3*i] = C[2]*(Xd[3]+Ud[1]) + C[5]*(Xd[4]+Ud[3]) + C[8]*(Xd[5]+Ud[5]);
  }
...
void compute_normal_rate_theta( TacsScalar dr_xi[], 
  ...
  dr_xi[2] = Na*Ctn[2] + N*(Ctn_xi[2] + C1tn[2]*Uxd[6] + C2tn[2]*Uxd[8] + C3tn[2]*Uxd[10]);

  dr_eta[0] = Nb*Ctn[0] + N*(Ctn_eta[0] + C1tn[0]*Uxd[7] + C2tn[0]*Uxd[9] + C3tn[0]*Uxd[11]);
  dr_eta[1] = Nb*Ctn[1] + N*(Ctn_eta[1] + C1tn[1]*Uxd[7] + C2tn[1]*Uxd[9] + C3tn[1]*Uxd[11]);
  dr_eta[2] = Nb*Ctn[2] + N*(Ctn_eta[2] + C1tn[2]*Uxd[7] + C2tn[2]*Uxd[9] + C3tn[2]*Uxd[11]);
}
...
/*
  ...
  and its derivative
  n_eta, dn_eta: the derivative of the shell normal w.r.t. the coordinate line eta
  and its derivative
...
void add_large_rot_stress_bmat( TacsScalar matrix[], const int num_points, 
  ...
  TacsScalar C112n[3], C113n[3], C122n[3], C123n[3], C133n[3], C223n[3], C233n[3];
