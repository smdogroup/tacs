...
/*
  ...
  [1] https://docs.plm.automation.siemens.com/data_services/resources/nxnastran/10/help/en_US/custom/advanced_nonlinear/advanced_nonlinear_tmg.pdf
  [2] http://www2.me.rochester.edu/courses/ME204/nx_help/index.html#uid:id1322317
...
/*
  Create the RBE3.
*/
TACSRBE3::TACSRBE3( int numNodes, int _dep_dof_constrained[],
...
const char * TACSRBE3::extraNames[] = { "fx", "fy", "fz",
				                        "mx", "my", "mz" };
...
*/
void TACSRBE3::addResidual( int elemIndex, double time,
  ...
  /* The next 6*NUM_INDEP_NODES equations distribute the reaction forces
     ...
     uniform linear acceleration for all points in the body, given by the equation below:
        a = F_cg/W
     Where, W is the sum of all independent nodal weights (masses). The force on each
     node can be found by multiplying the acceleration by the nodal weight (mass):
        F_n = w[n]/W*F_cg
     The moment distribution can be treated in a similar way, in this case we know that
     a moment applied to the cg of a rigid body produces uniform rotational acceleration
     of all point about the cg:
        alpha = Icg^-1*M_cg
     Where Icg is the moment of inertia tensor for all nodes about the cg. To get the linear
     acceleration of any point due to the rotational acceleration we use the following equation:
        a_n = alpha x rcn = - rcn x Icg^-1 * M_cg
     Where rcn is the vector pointing from the cg to node n. Finally the force contribution can
     be found by multiplying by the weight (mass):
        F_n = - w[n] * rcn x Icg^-1 * M_cg
     Putting the force and moment contributions together gives full transfer equation:
        F_n = w[n]/W * F_cg - w[n] * rcn x Icg^-1 * M_cg
     The last step is to write the force and moment terms at the cg in terms of the forces
     we have (at the dependent node):
        F_n = w[n]/W * F_0 - w[n] * rcn x Icg^-1 * (M_0 + rco x F0)

     This formula can be generalized to include point moments using the approach defined in ref [1].
  ...
  for ( int n = 0; n < NUM_INDEP_NODES; n++ ){
    ...
    if (indep_dof_constrained[n][0]){
        res[0]  = -w[n]/W[0] * F0[0];
        res[0] += -w[n]*(Jcg[1][0]*rnc[2]-Jcg[2][0]*rnc[1])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
        res[0] += -w[n]*(Jcg[1][1]*rnc[2]-Jcg[2][1]*rnc[1])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
        res[0] += -w[n]*(Jcg[1][2]*rnc[2]-Jcg[2][2]*rnc[1])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
    }

    // Fy
    if (indep_dof_constrained[n][1]){
        res[1]  = -w[n]/W[1] * F0[1];
        res[1] += -w[n]*(Jcg[2][0]*rnc[0]-Jcg[0][0]*rnc[2])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
        res[1] += -w[n]*(Jcg[2][1]*rnc[0]-Jcg[0][1]*rnc[2])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
        res[1] += -w[n]*(Jcg[2][2]*rnc[0]-Jcg[0][2]*rnc[2])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
    }

    // Fz
    if (indep_dof_constrained[n][2]){
        res[2]  = -w[n]/W[2] * F0[2];
        res[2] += -w[n]*(Jcg[0][0]*rnc[1]-Jcg[1][0]*rnc[0])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
        res[2] += -w[n]*(Jcg[0][1]*rnc[1]-Jcg[1][1]*rnc[0])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
        res[2] += -w[n]*(Jcg[0][2]*rnc[1]-Jcg[1][2]*rnc[0])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
    }

    // Moments
    // Mx
    if (indep_dof_constrained[n][3]){
        res[3] = -w[n]*Lc*Lc*Jcg[0][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
        res[3] += -w[n]*Lc*Lc*Jcg[0][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
        res[3] += -w[n]*Lc*Lc*Jcg[0][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
    }

    // My
    if (indep_dof_constrained[n][4]){
        res[4] = -w[n]*Lc*Lc*Jcg[1][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
        res[4] += -w[n]*Lc*Lc*Jcg[1][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
        res[4] += -w[n]*Lc*Lc*Jcg[1][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
    }

    if (indep_dof_constrained[n][5]){
        res[5] = -w[n]*Lc*Lc*Jcg[2][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
        res[5] += -w[n]*Lc*Lc*Jcg[2][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
        res[5] += -w[n]*Lc*Lc*Jcg[2][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
    }
  ...
  /* The last six equations enforce that the dependent nodes displacements are
     the weighted average of the independent nodes. I don't know that an intuitive
     interpretation of this equation exists, but we can take the transpose of the load
     transfer matrix above (using the Lagrange multiplier method) to figure out what the
     coefficients need to be. Only add equations for constrained DOFs */
  ...
  for ( int n = 0; n < NUM_INDEP_NODES; n++ ){
    ...
    if (dep_dof_constrained[1]){
        res[1] +=  -w[n]*(Jcg[1][0]*rnc[2]-Jcg[2][0]*rnc[1])*-rco[2]*un[0] \
    ...
    if (dep_dof_constrained[2]){
        res[2] +=  -w[n]*(Jcg[1][0]*rnc[2]-Jcg[2][0]*rnc[1])*rco[1]*un[0] \
  ...
  // Rescale all equations so they are on the same order of magnitude as the global stiffness matrix
...
*/
void TACSRBE3::addJacobian( int elemIndex, double time,
                            TacsScalar alpha,
                            TacsScalar beta,
                            TacsScalar gamma,
                            const TacsScalar Xpts[],
  ...
  // NOTE: For now we will populate the matrix as if the problem is fully constrained,
  //       We will wait until the last step to zero out the necessary components.
  // We'll take advantage of the symmetry of

  /* The first 6 equations enforce equilibrium between the forces at the rbe3 dep node */
  ...
  // The next 6*NUM_INDEP_NODES equations distribute the forces at the dep node among the dependent nodes
  X0 = &Xpts[0]; // dependent node location
  // vector pointing from cg to ref node
  TacsScalar rco[3], rnc[3];
  for (int i = 0 ; i < 3; i++){
...
/*
  ...
  variables and elements.
*/
void TACSRBE3::addAdjResXptProduct( int elemIndex, double time,
                                    TacsScalar scale,
  ...
  for ( int k = 0; k < 3 * (NUM_NODES - 1); k++ ){
      ...
      for ( int n = 0; n < NUM_INDEP_NODES; n++ ){
        ...
        if (indep_dof_constrained[n][0]){
            res[0]  = 0.0;
            res[0] += -w[n]*(sJcg[1][0]*rnc[2]-sJcg[2][0]*rnc[1])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[0] += -w[n]*(sJcg[1][1]*rnc[2]-sJcg[2][1]*rnc[1])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[0] += -w[n]*(sJcg[1][2]*rnc[2]-sJcg[2][2]*rnc[1])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[0] += -w[n]*(Jcg[1][0]*srnc[2]-Jcg[2][0]*srnc[1])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[0] += -w[n]*(Jcg[1][1]*srnc[2]-Jcg[2][1]*srnc[1])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[0] += -w[n]*(Jcg[1][2]*srnc[2]-Jcg[2][2]*srnc[1])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[0] += -w[n]*(Jcg[1][0]*rnc[2]-Jcg[2][0]*rnc[1])*(srco[1]*F0[2]-srco[2]*F0[1]);
            res[0] += -w[n]*(Jcg[1][1]*rnc[2]-Jcg[2][1]*rnc[1])*(-srco[0]*F0[2]+srco[2]*F0[0]);
            res[0] += -w[n]*(Jcg[1][2]*rnc[2]-Jcg[2][2]*rnc[1])*(srco[0]*F0[1]-srco[1]*F0[0]);
        }

        // Fy
        if (indep_dof_constrained[n][1]){
            res[1]  = 0.0;
            res[1] += -w[n]*(sJcg[2][0]*rnc[0]-sJcg[0][0]*rnc[2])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[1] += -w[n]*(sJcg[2][1]*rnc[0]-sJcg[0][1]*rnc[2])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[1] += -w[n]*(sJcg[2][2]*rnc[0]-sJcg[0][2]*rnc[2])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[1] += -w[n]*(Jcg[2][0]*srnc[0]-Jcg[0][0]*srnc[2])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[1] += -w[n]*(Jcg[2][1]*srnc[0]-Jcg[0][1]*srnc[2])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[1] += -w[n]*(Jcg[2][2]*srnc[0]-Jcg[0][2]*srnc[2])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[1] += -w[n]*(Jcg[2][0]*rnc[0]-Jcg[0][0]*rnc[2])*(srco[1]*F0[2]-srco[2]*F0[1]);
            res[1] += -w[n]*(Jcg[2][1]*rnc[0]-Jcg[0][1]*rnc[2])*(-srco[0]*F0[2]+srco[2]*F0[0]);
            res[1] += -w[n]*(Jcg[2][2]*rnc[0]-Jcg[0][2]*rnc[2])*(srco[0]*F0[1]-srco[1]*F0[0]);
        }

        // Fz
        if (indep_dof_constrained[n][2]){
            res[2]  = 0.0;
            res[2] += -w[n]*(sJcg[0][0]*rnc[1]-sJcg[1][0]*rnc[0])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[2] += -w[n]*(sJcg[0][1]*rnc[1]-sJcg[1][1]*rnc[0])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[2] += -w[n]*(sJcg[0][2]*rnc[1]-sJcg[1][2]*rnc[0])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[2] += -w[n]*(Jcg[0][0]*srnc[1]-Jcg[1][0]*srnc[0])*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[2] += -w[n]*(Jcg[0][1]*srnc[1]-Jcg[1][1]*srnc[0])*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[2] += -w[n]*(Jcg[0][2]*srnc[1]-Jcg[1][2]*srnc[0])*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[2] += -w[n]*(Jcg[0][0]*rnc[1]-Jcg[1][0]*rnc[0])*(srco[1]*F0[2]-srco[2]*F0[1]);
            res[2] += -w[n]*(Jcg[0][1]*rnc[1]-Jcg[1][1]*rnc[0])*(-srco[0]*F0[2]+srco[2]*F0[0]);
            res[2] += -w[n]*(Jcg[0][2]*rnc[1]-Jcg[1][2]*rnc[0])*(srco[0]*F0[1]-srco[1]*F0[0]);
        }

        // Moments
        // Mx
        if (indep_dof_constrained[n][3]){
            res[3] = 2.0*-w[n]*Lc*sLc*Jcg[0][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[3] += -w[n]*Lc*Lc*sJcg[0][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[3] += -w[n]*Lc*Lc*Jcg[0][0]*(srco[1]*F0[2]-srco[2]*F0[1]);
            res[3] += 2.0*-w[n]*Lc*sLc*Jcg[0][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[3] += -w[n]*Lc*Lc*sJcg[0][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[3] += -w[n]*Lc*Lc*Jcg[0][1]*(-srco[0]*F0[2]+srco[2]*F0[0]);
            res[3] += 2.0*-w[n]*Lc*sLc*Jcg[0][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[3] += -w[n]*Lc*Lc*sJcg[0][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[3] += -w[n]*Lc*Lc*Jcg[0][2]*(srco[0]*F0[1]-srco[1]*F0[0]);
        }

        // My
        if (indep_dof_constrained[n][4]){
            res[4] = 2.0*-w[n]*Lc*sLc*Jcg[1][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[4] += -w[n]*Lc*Lc*sJcg[1][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[4] += -w[n]*Lc*Lc*Jcg[1][0]*(srco[1]*F0[2]-srco[2]*F0[1]);
            res[4] += 2.0*-w[n]*Lc*sLc*Jcg[1][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[4] += -w[n]*Lc*Lc*sJcg[1][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[4] += -w[n]*Lc*Lc*Jcg[1][1]*(-srco[0]*F0[2]+srco[2]*F0[0]);
            res[4] += 2.0*-w[n]*Lc*sLc*Jcg[1][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[4] += -w[n]*Lc*Lc*sJcg[1][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[4] += -w[n]*Lc*Lc*Jcg[1][2]*(srco[0]*F0[1]-srco[1]*F0[0]);
        }

        if (indep_dof_constrained[n][5]){
            res[5] = 2.0*-w[n]*Lc*sLc*Jcg[2][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[5] += -w[n]*Lc*Lc*sJcg[2][0]*(M0[0]+rco[1]*F0[2]-rco[2]*F0[1]);
            res[5] += -w[n]*Lc*Lc*Jcg[2][0]*(srco[1]*F0[2]-srco[2]*F0[1]);
            res[5] += 2.0*-w[n]*Lc*sLc*Jcg[2][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[5] += -w[n]*Lc*Lc*sJcg[2][1]*(M0[1]-rco[0]*F0[2]+rco[2]*F0[0]);
            res[5] += -w[n]*Lc*Lc*Jcg[2][1]*(-srco[0]*F0[2]+srco[2]*F0[0]);
            res[5] += 2.0*-w[n]*Lc*sLc*Jcg[2][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[5] += -w[n]*Lc*Lc*sJcg[2][2]*(M0[2]+rco[0]*F0[1]-rco[1]*F0[0]);
            res[5] += -w[n]*Lc*Lc*Jcg[2][2]*(srco[0]*F0[1]-srco[1]*F0[0]);
        }
      ...
      for ( int n = 0; n < NUM_INDEP_NODES; n++ ){
        ...
        if (dep_dof_constrained[0]){
            res[0] += 0.0 \
                +  -w[n]*(sJcg[1][1]*rnc[2]-sJcg[2][1]*rnc[1])*rco[2]*un[0] \
                + -w[n]*(sJcg[1][2]*rnc[2]-sJcg[2][2]*rnc[1])*-rco[1]*un[0] \
                + -w[n]*(sJcg[2][1]*rnc[0]-sJcg[0][1]*rnc[2])*rco[2]*un[1] \
                + -w[n]*(sJcg[2][2]*rnc[0]-sJcg[0][2]*rnc[2])*-rco[1]*un[1] \
                + -w[n]*(sJcg[0][1]*rnc[1]-sJcg[1][1]*rnc[0])*rco[2]*un[2] \
                + -w[n]*(sJcg[0][2]*rnc[1]-sJcg[1][2]*rnc[0])*-rco[1]*un[2] \
                +  -w[n]*(Jcg[1][1]*srnc[2]-Jcg[2][1]*srnc[1])*rco[2]*un[0] \
                + -w[n]*(Jcg[1][2]*srnc[2]-Jcg[2][2]*srnc[1])*-rco[1]*un[0] \
                + -w[n]*(Jcg[2][1]*srnc[0]-Jcg[0][1]*srnc[2])*rco[2]*un[1] \
                + -w[n]*(Jcg[2][2]*srnc[0]-Jcg[0][2]*srnc[2])*-rco[1]*un[1] \
                + -w[n]*(Jcg[0][1]*srnc[1]-Jcg[1][1]*srnc[0])*rco[2]*un[2] \
                + -w[n]*(Jcg[0][2]*srnc[1]-Jcg[1][2]*srnc[0])*-rco[1]*un[2] \
                +  -w[n]*(Jcg[1][1]*rnc[2]-Jcg[2][1]*rnc[1])*srco[2]*un[0] \
                + -w[n]*(Jcg[1][2]*rnc[2]-Jcg[2][2]*rnc[1])*-srco[1]*un[0] \
                + -w[n]*(Jcg[2][1]*rnc[0]-Jcg[0][1]*rnc[2])*srco[2]*un[1] \
                + -w[n]*(Jcg[2][2]*rnc[0]-Jcg[0][2]*rnc[2])*-srco[1]*un[1] \
                + -w[n]*(Jcg[0][1]*rnc[1]-Jcg[1][1]*rnc[0])*srco[2]*un[2] \
                + -w[n]*(Jcg[0][2]*rnc[1]-Jcg[1][2]*rnc[0])*-srco[1]*un[2] \
        ...
        if (dep_dof_constrained[1]){
            res[1] +=  -w[n]*(sJcg[1][0]*rnc[2]-sJcg[2][0]*rnc[1])*-rco[2]*un[0] \
                + -w[n]*(sJcg[1][2]*rnc[2]-sJcg[2][2]*rnc[1])*rco[0]*un[0] \
                + 0.0 \
                + -w[n]*(sJcg[2][0]*rnc[0]-sJcg[0][0]*rnc[2])*-rco[2]*un[1] \
                + -w[n]*(sJcg[2][2]*rnc[0]-sJcg[0][2]*rnc[2])*rco[0]*un[1] \
                + -w[n]*(sJcg[0][0]*rnc[1]-sJcg[1][0]*rnc[0])*-rco[2]*un[2] \
                + -w[n]*(sJcg[0][2]*rnc[1]-sJcg[1][2]*rnc[0])*rco[0]*un[2] \
                - w[n]*(Jcg[1][0]*srnc[2]-Jcg[2][0]*srnc[1])*-rco[2]*un[0] \
                + -w[n]*(Jcg[1][2]*srnc[2]-Jcg[2][2]*srnc[1])*rco[0]*un[0] \
                + -w[n]*(Jcg[2][0]*srnc[0]-Jcg[0][0]*srnc[2])*-rco[2]*un[1] \
                + -w[n]*(Jcg[2][2]*srnc[0]-Jcg[0][2]*srnc[2])*rco[0]*un[1] \
                + -w[n]*(Jcg[0][0]*srnc[1]-Jcg[1][0]*srnc[0])*-rco[2]*un[2] \
                + -w[n]*(Jcg[0][2]*srnc[1]-Jcg[1][2]*srnc[0])*rco[0]*un[2] \
                - w[n]*(Jcg[1][0]*rnc[2]-Jcg[2][0]*rnc[1])*-srco[2]*un[0] \
                + -w[n]*(Jcg[1][2]*rnc[2]-Jcg[2][2]*rnc[1])*srco[0]*un[0] \
                + -w[n]*(Jcg[2][0]*rnc[0]-Jcg[0][0]*rnc[2])*-srco[2]*un[1] \
                + -w[n]*(Jcg[2][2]*rnc[0]-Jcg[0][2]*rnc[2])*srco[0]*un[1] \
                + -w[n]*(Jcg[0][0]*rnc[1]-Jcg[1][0]*rnc[0])*-srco[2]*un[2] \
                + -w[n]*(Jcg[0][2]*rnc[1]-Jcg[1][2]*rnc[0])*srco[0]*un[2] \
        ...
        if (dep_dof_constrained[2]){
            res[2] +=  -w[n]*(sJcg[1][0]*rnc[2]-sJcg[2][0]*rnc[1])*rco[1]*un[0] \
                       +  -w[n]*(sJcg[1][1]*rnc[2]-sJcg[2][1]*rnc[1])*-rco[0]*un[0] \
                       + -w[n]*(sJcg[2][0]*rnc[0]-sJcg[0][0]*rnc[2])*rco[1]*un[1] \
                       + -w[n]*(sJcg[2][1]*rnc[0]-sJcg[0][1]*rnc[2])*-rco[0]*un[1] \
                       + 0.0 \
                       + -w[n]*(sJcg[0][0]*rnc[1]-sJcg[1][0]*rnc[0])*rco[1]*un[2] \
                       + -w[n]*(sJcg[0][1]*rnc[1]-sJcg[1][1]*rnc[0])*-rco[0]*un[2] \
                       - w[n]*(Jcg[1][0]*srnc[2]-Jcg[2][0]*srnc[1])*rco[1]*un[0] \
                       +  -w[n]*(Jcg[1][1]*srnc[2]-Jcg[2][1]*srnc[1])*-rco[0]*un[0] \
                       + -w[n]*(Jcg[2][0]*srnc[0]-Jcg[0][0]*srnc[2])*rco[1]*un[1] \
                       + -w[n]*(Jcg[2][1]*srnc[0]-Jcg[0][1]*srnc[2])*-rco[0]*un[1] \
                       + -w[n]*(Jcg[0][0]*srnc[1]-Jcg[1][0]*srnc[0])*rco[1]*un[2] \
                       + -w[n]*(Jcg[0][1]*srnc[1]-Jcg[1][1]*srnc[0])*-rco[0]*un[2] \
                       - w[n]*(Jcg[1][0]*rnc[2]-Jcg[2][0]*rnc[1])*srco[1]*un[0] \
                       +  -w[n]*(Jcg[1][1]*rnc[2]-Jcg[2][1]*rnc[1])*-srco[0]*un[0] \
                       + -w[n]*(Jcg[2][0]*rnc[0]-Jcg[0][0]*rnc[2])*srco[1]*un[1] \
                       + -w[n]*(Jcg[2][1]*rnc[0]-Jcg[0][1]*rnc[2])*-srco[0]*un[1] \
                       + -w[n]*(Jcg[0][0]*rnc[1]-Jcg[1][0]*rnc[0])*srco[1]*un[2] \
                       + -w[n]*(Jcg[0][1]*rnc[1]-Jcg[1][1]*rnc[0])*-srco[0]*un[2] \
...
void TACSRBE3::getCG( TacsScalar Xcg[],
                      TacsScalar W[],
                      const double w[],
                      const TacsScalar Xpts[] ){
...
void TACSRBE3::getCGSens( TacsScalar sXcg[],
                          TacsScalar Xcg[],
                          TacsScalar W[],
                          const double w[],
                          const TacsScalar Xpts[],
...
TacsScalar TACSRBE3::getMomentsOfInertia( TacsScalar IcgInv[3][3],
  ...
  // Check if the determinate is zero, if so at least one of the principal moment of inertias is zero ( the points are colinear)
...
TacsScalar TACSRBE3::getMomentsOfInertiaSens( TacsScalar sIcgInv[3][3],
  ...
  for ( int n = 0; n < NUM_INDEP_NODES; n++ ){
    ...
    Lc += vec3Dot(r, r) / TacsScalar(NUM_INDEP_NODES);
    *sLc += 2.0 * (r[0]*sr[0]+r[1]*sr[1]+r[2]*sr[2]) / TacsScalar(NUM_INDEP_NODES);
  }
  Lc = sqrt(Lc);
  *sLc = *sLc / 2.0 / Lc;
  // If Lc is zero, set it to 1.0
  ...
  for ( int n = 0; n < NUM_INDEP_NODES; n++ ){
    ...
    sIcg[0] += 2.0 * (c[2] * r[1] * sr[1] + c[1] * r[2] * sr[2] + c[3] * Lc * *sLc); // Ixx
    sIcg[1] += c[2] * -(sr[0] * r[1] + r[0] * sr[1]); // Ixy
    sIcg[2] += c[1] * -(sr[0] * r[2] + r[0] * sr[2]); // Ixz
    sIcg[3] = sIcg[1]; // Iyx
    sIcg[4] += 2.0 * (c[2] * r[0] * sr[0] + c[0] * r[2] * sr[2] + c[4] * Lc * *sLc); // Iyy
    sIcg[5] += c[0] * -(sr[1] * r[2] + r[1] * sr[2]); // Iyz
    sIcg[6] = sIcg[2]; // Izx
    sIcg[7] = sIcg[5]; // Izy
    sIcg[8] += 2.0 * (c[1] * r[0] * sr[0] + c[0] * r[1] * sr[1] + c[5] * Lc * *sLc); // Izz
...
void TACSRBE3::getMaskedVars( TacsScalar maskedVars[],
                              const TacsScalar vars[] ){
