...
class TACSShellQuadBasis {
 public:
  ...
  static const int G11_OFFSET =
    NUM_G11_TYING_POINTS;
  static const int G22_OFFSET =
    (NUM_G11_TYING_POINTS +
  ...
  */
  static inline void getTyingPoint( int ty_index, double pt[] ){
    ...
    if (field == TACS_SHELL_G11_COMPONENT || field == TACS_SHELL_G13_COMPONENT){ // g11 or g13
      pt[0] = ty_knots_reduced[ty % (order - 1)];
      pt[1] = ty_knots_order[ty / (order - 1)];
    }
    else if (field == TACS_SHELL_G22_COMPONENT || field == TACS_SHELL_G23_COMPONENT){ // g22 or g23
  ...
  */
  static inline int getNumTyingPoints( const int field ){
    if (field == TACS_SHELL_G11_COMPONENT){ return NUM_G11_TYING_POINTS; }
    else if (field == TACS_SHELL_G22_COMPONENT){ return NUM_G22_TYING_POINTS; }
    else if (field == TACS_SHELL_G12_COMPONENT){ return NUM_G12_TYING_POINTS; }
    else if (field == TACS_SHELL_G23_COMPONENT){ return NUM_G23_TYING_POINTS; }
    else if (field == TACS_SHELL_G13_COMPONENT){ return NUM_G13_TYING_POINTS; }
    return 0;
  }
    ...
    for ( int field1 = 0; field1 < num_tying_fields; field1++ ){
      ...
      for ( int k1 = 0; k1 < npts1; k1++, N1++ ){
        ...
        for ( int field2 = 0; field2 < num_tying_fields; field2++ ){
          ...
          const TacsScalar value = d2gty[num_strains*index[field1] + index[field2]];
