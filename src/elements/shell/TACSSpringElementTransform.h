...
class TACSSpringTransform : public TACSObject {
  ...
  */
  virtual void computeTransform( const TacsScalar Xpts[],
                                 TacsScalar T[] ) = 0;
  ...
  /*
    ...
    nodal coordinate.
  */
  virtual void computeTransformXptSens( const TacsScalar Xpts[],
                                        int component,
                                        TacsScalar t[],
                                        TacsScalar tSens[] ){
...
class TACSSpringIdentityTransform : public TACSSpringTransform {
 public:
  ...
  void computeTransform( const TacsScalar Xpts[],
                         TacsScalar T[] ){
  ...
  */
  void computeTransform( const TacsScalar Xpts[],
    ...
    if (vec3Dot(ref_dir, &t[0]) == 1.0){
      fprintf(stderr,
              "TACSSpringElement: Error, reference axis and spring axis are parallel\n");
    }
    else {
  ...
  void computeTransformXptSens( const TacsScalar Xpts[],
                                int component,
                                TacsScalar t[],
                                TacsScalar tSens[] ){
...
class TACSSpringRefFrameTransform : public TACSSpringTransform {
 public:
  TACSSpringRefFrameTransform( TacsScalar _ref_axis_i[], TacsScalar _ref_axis_j[] ){
