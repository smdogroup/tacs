  ...
  static void addComputeTyingStrainHessianDeriv(
    ...
    for (int i1 = 0; i1 < basis::NUM_TYING_POINTS; i1++, n01 += 3, Xxi1 += 6,
      ...
      if (f1 == TACS_SHELL_G11_COMPONENT) {
        ...
        d2Uxid[0] = alpha * detyd[i1];
        d2Uxid[14] = alpha * detyd[i1];
        d2Uxid[28] = alpha * detyd[i1];
      } else if (f1 == TACS_SHELL_G22_COMPONENT) {
        // Compute g22 = e2^{T}*G*e2
        dUxi1[0] = 0.0;
        dUxi1[1] = (Xxi1[1] + Uxi1[1]);
        ...
        d2Uxid[7] = alpha * detyd[i1];
        d2Uxid[21] = alpha * detyd[i1];
        d2Uxid[35] = alpha * detyd[i1];
      } else if (f1 == TACS_SHELL_G12_COMPONENT) {
        // Compute g12 = e2^{T}*G*e1
        dUxi1[0] = 0.5 * (Xxi1[1] + Uxi1[1]);
        dUxi1[1] = 0.5 * (Xxi1[0] + Uxi1[0]);
        ...
        d2Uxid[1] = 0.5 * alpha * detyd[i1];
        d2Uxid[6] = 0.5 * alpha * detyd[i1];
        d2Uxid[15] = 0.5 * alpha * detyd[i1];
        d2Uxid[20] = 0.5 * alpha * detyd[i1];
        d2Uxid[29] = 0.5 * alpha * detyd[i1];
        d2Uxid[34] = 0.5 * alpha * detyd[i1];
      } else {
        TacsScalar dd1[3 * basis::NUM_NODES];
        memset(dd1, 0, 3 * basis::NUM_NODES * sizeof(TacsScalar));
        ...
        const TacsScalar *etdown = &d2etydown[3 * basis::NUM_NODES * i1];
        const TacsScalar *etup = &d2etyup[3 * basis::NUM_NODES * i1];
        const TacsScalar *etdownd = &d2etydownd[3 * basis::NUM_NODES * i1];
        const TacsScalar *etupd = &d2etyupd[3 * basis::NUM_NODES * i1];
        for (int i = 0; i < 3 * basis::NUM_NODES; i++) {
          for (int j = 0; j < 3 * basis::NUM_NODES; j++) {
            d2d[3 * basis::NUM_NODES * i + j] +=
