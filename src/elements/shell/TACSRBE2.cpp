...
/*
  ...
  [1] http://www2.me.rochester.edu/courses/ME204/nx_help/index.html#uid:id1322317
...
const char * TACSRBE2::extraNames[] = { "fx", "fy", "fz",
				                        "mx", "my", "mz" };
...
*/
void TACSRBE2::addResidual( int elemIndex, double time,
  ...
  for (int n = 0; n < NUM_DEP_NODES; n++){
      ...
      // Moment equilibrium, we have to account for the couple portion as well as the moment arm about the dep node
  ...
  // The next 6*NUM_DEP_NODES simply enforce that the forces on dep nodes match whats stored in the Lagrange multipliers
  Fn = &maskedLM[0];
  Mn = &maskedLM[3];
  Xn = &Xpts[3];
  for ( int n = 0; n < NUM_DEP_NODES; n++ ){
  ...
  // The last 6*NUM_DEP_NODES equations enforce that the dependent nodes displacements are
  // rigid transformations of the independent one (only add equations for constrianed dofs)
...
*/
void TACSRBE2::addJacobian( int elemIndex, double time,
                            TacsScalar alpha,
                            TacsScalar beta,
                            TacsScalar gamma,
                            const TacsScalar Xpts[],
...
/*
  ...
  variables and elements.
*/
void TACSRBE2::addAdjResXptProduct( int elemIndex, double time,
                                    TacsScalar scale,
  ...
  // Store the last 6*NUM_DEP_NODES variables (Lagrange multipliers) as a force and moment
  ...
  for ( int k = 0; k < 3 * (NUM_DEP_NODES + 1); k++ ){
      ...
      for (int n = 0; n < NUM_DEP_NODES; n++){
          ...
          // Moment equilibrium, we have to account for the couple portion as well as the moment arm about the dep node
          res[3] -= C1*( - (sXn[2] - sX0[2]) * Fn[1] + (sXn[1] - sX0[1]) * Fn[2] ); // Mx
          res[4] -= C1*( + (sXn[2] - sX0[2]) * Fn[0] - (sXn[0] - sX0[0]) * Fn[2] ); // My
          res[5] -= C1*( - (sXn[1] - sX0[1]) * Fn[0] + (sXn[0] - sX0[0]) * Fn[1] ); // Mz
      ...
      // The last 6*NUM_DEP_NODES equations enforce that the dependent nodes displacements are
      // rigid transformations of the independent on
      t0 = &vars[3]; // indep node rotations
      sXn = &sXpts[3];
