...
/*
  ...
  direction.
*/
void large_rot_strain( TacsScalar strain[], 
                       const TacsScalar Ux[],
...
*/
void large_rot_bend_strain( TacsScalar strain[], 
                            const TacsScalar Ux[],
                            const TacsScalar Uxd[], 
                            const TacsScalar C[],
                            const TacsScalar Ct[],
...
*/
void large_rot_bend_strain_sens( TacsScalar strain[], TacsScalar dstrain[],
                                 const TacsScalar Ux[], const TacsScalar Uxd[],
                                 const TacsScalar C[], const TacsScalar Ct[],
                                 const TacsScalar t[], const TacsScalar dt[],
                                 const TacsScalar tx[], const TacsScalar dtx[],
  ...
  for ( int m = 0; m < tying_order-1; m++ ){
    for ( int n = 0; n < tying_order; n++ ){
      ...
      for ( int k = 0; k < 3; k++ ){
        ...
        rt[3*k+1] = Ca[3]*normal[0] + Ca[4]*normal[1] + Ca[5]*normal[2];
        rt[3*k+2] = Ca[6]*normal[0] + Ca[7]*normal[1] + Ca[8]*normal[2];
      }
      ...
      for ( int i = 0; i < order*order; i++ ){
        ...
        b23[3] = 0.5*N[i]*((Xd[3] + Ud[1])*rt[0] + (Xd[4] + Ud[3])*rt[1] + (Xd[5] + Ud[5])*rt[2]);
        b23[4] = 0.5*N[i]*((Xd[3] + Ud[1])*rt[3] + (Xd[4] + Ud[3])*rt[4] + (Xd[5] + Ud[5])*rt[5]);
        b23[5] = 0.5*N[i]*((Xd[3] + Ud[1])*rt[6] + (Xd[4] + Ud[3])*rt[7] + (Xd[5] + Ud[5])*rt[8]);
        b23 += 6;
      }
    }
  }
  ...
  for ( int m = 0; m < tying_order; m++ ){
    for ( int n = 0; n < tying_order-1; n++ ){
      ...
      for ( int i = 0; i < order*order; i++ ){
        ...
        b13[3] = 0.5*N[i]*((Xd[0] + Ud[0])*rt[0] + (Xd[1] + Ud[2])*rt[1] + (Xd[2] + Ud[4])*rt[2]);
        b13[4] = 0.5*N[i]*((Xd[0] + Ud[0])*rt[3] + (Xd[1] + Ud[2])*rt[4] + (Xd[2] + Ud[4])*rt[5]);
        b13[5] = 0.5*N[i]*((Xd[0] + Ud[0])*rt[6] + (Xd[1] + Ud[2])*rt[7] + (Xd[2] + Ud[4])*rt[8]);
        b13 += 6;
      }
    }
  }
  ...
  for ( int m = 0; m < tying_order-1; m++ ){
    for ( int n = 0; n < tying_order; n++ ){
      ...
      for ( int k = 0; k < 6; k++ ){
        ...
        rtt[3*k+1] = Ca[3]*normal[0] + Ca[4]*normal[1] + Ca[5]*normal[2];
        rtt[3*k+2] = Ca[6]*normal[0] + Ca[7]*normal[1] + Ca[8]*normal[2];
      }
      ...
      for ( int i = 0; i < order*order; i++ ){
        ...
        for ( int j = 0; j <= i; j++ ){
          ...
          n23[0] = 0.5*N[j]*N[i]*((Xd[3] + Ud[1])*rtt[0]  + (Xd[4] + Ud[3])*rtt[1]  + (Xd[5] + Ud[5])*rtt[2]);
          n23[1] = 0.5*N[j]*N[i]*((Xd[3] + Ud[1])*rtt[3]  + (Xd[4] + Ud[3])*rtt[4]  + (Xd[5] + Ud[5])*rtt[5]);
          n23[2] = 0.5*N[j]*N[i]*((Xd[3] + Ud[1])*rtt[6]  + (Xd[4] + Ud[3])*rtt[7]  + (Xd[5] + Ud[5])*rtt[8]);
          n23[3] = 0.5*N[j]*N[i]*((Xd[3] + Ud[1])*rtt[9]  + (Xd[4] + Ud[3])*rtt[10] + (Xd[5] + Ud[5])*rtt[11]);
          n23[4] = 0.5*N[j]*N[i]*((Xd[3] + Ud[1])*rtt[12] + (Xd[4] + Ud[3])*rtt[13] + (Xd[5] + Ud[5])*rtt[14]);
          n23[5] = 0.5*N[j]*N[i]*((Xd[3] + Ud[1])*rtt[15] + (Xd[4] + Ud[3])*rtt[16] + (Xd[5] + Ud[5])*rtt[17]);
          n23 += 6;
        }
      }
    }
    ...
    for ( int n = 0; n < tying_order-1; n++ ){
      ...
      for ( int i = 0; i < order*order; i++ ){
        ...
        for ( int j = 0; j <= i; j++ ){
          ...
          n13[0] = 0.5*N[j]*N[i]*((Xd[0] + Ud[0])*rtt[0]  + (Xd[1] + Ud[2])*rtt[1]  + (Xd[2] + Ud[4])*rtt[2]);
          n13[1] = 0.5*N[j]*N[i]*((Xd[0] + Ud[0])*rtt[3]  + (Xd[1] + Ud[2])*rtt[4]  + (Xd[2] + Ud[4])*rtt[5]);
          n13[2] = 0.5*N[j]*N[i]*((Xd[0] + Ud[0])*rtt[6]  + (Xd[1] + Ud[2])*rtt[7]  + (Xd[2] + Ud[4])*rtt[8]);
          n13[3] = 0.5*N[j]*N[i]*((Xd[0] + Ud[0])*rtt[9]  + (Xd[1] + Ud[2])*rtt[10] + (Xd[2] + Ud[4])*rtt[11]);
          n13[4] = 0.5*N[j]*N[i]*((Xd[0] + Ud[0])*rtt[12] + (Xd[1] + Ud[2])*rtt[13] + (Xd[2] + Ud[4])*rtt[14]);
          n13[5] = 0.5*N[j]*N[i]*((Xd[0] + Ud[0])*rtt[15] + (Xd[1] + Ud[2])*rtt[16] + (Xd[2] + Ud[4])*rtt[17]);
          n13 += 6;
        }
      }
    }
