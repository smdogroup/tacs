...
class TACS2DElement : public TACSElement {
 public:
  ...
  TACS2DElement( PlaneStressStiffness * _stiff,
...
*/
template <int NUM_NODES>
const char * TACS2DElement<NUM_NODES>::displacementName( int i ) const {
  if (i >= 0 && i < NUM_DISPS){
    return dispNames[i];
  }
  return NULL;
...
template <int NUM_NODES>
const char * TACS2DElement<NUM_NODES>::stressName( int i ) const { 
...
template <int NUM_NODES>
const char * TACS2DElement<NUM_NODES>::strainName( int i ) const {
  if (i >= 0 && i < NUM_STRESSES){
...
template <int NUM_NODES>
const char * TACS2DElement<NUM_NODES>::extraName( int i ) const {
  if (i >= 0 && i < NUM_EXTRAS){
    return extraNames[i];
  }
  return NULL;
...
*/
template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::numDisplacements() const { 
  return NUM_DISPS; 
}

template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::numStresses() const { 
  return NUM_STRESSES; 
}

template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::numNodes() const { 
  return NUM_NODES; 
}

template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::numVariables() const { 
  return NUM_VARIABLES; 
}

template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::numExtras() const { 
  return NUM_EXTRAS; 
}
...
*/
template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::ownsDesignVar( const int dvNum ) const { 
...
*/
template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::getDesignVarNums( int * dvNums, 
						int * dvIndex,
						int dvLen ) const {
...
*/
template <int NUM_NODES>
int TACS2DElement<NUM_NODES>::getNumDesignVars() const {
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getDesignVars( TacsScalar dvs[], 
					      int numDVs ) const {
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getDeformGradient( TacsScalar Ud[],
						  const TacsScalar J[],
						  const double Na[],
						  const double Nb[], 
						  const TacsScalar vars[] ){
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getDeformGradientSens( TacsScalar Ud[], 
						      TacsScalar UdSens[],
						      const TacsScalar J[],
...
void TACS2DElement<NUM_NODES>::getStrain( TacsScalar strain[], 
  ...
  // Compute the displacement gradient
  TacsScalar Ud[4];
  getDeformGradient(Ud, J, Na, Nb, vars);
  ...
  if (linear_strain){
    ...
    strain[3] = Ud[1] + Ud[2] + (Ud[0]*Ud[1] + Ud[2]*Ud[3]);
  }
}
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getBmat( TacsScalar B[], 
					const TacsScalar J[],
					const double Na[], 
					const double Nb[], 
					const TacsScalar vars[] ){
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::addGeoStiffness( TacsScalar mat[],
						TacsScalar h,
  ...
  if (!linear_strain){
	...
	mat[2*i + 2*j*NUM_VARIABLES] += h*stress[0]*Dxi*Dxj;
	mat[2*i+1 + 2*j*NUM_VARIABLES] += h*stress[2]*(Dxi*Dyj + Dxj*Dyi);
	
	mat[2*i + (2*j+1)*NUM_VARIABLES] += h*stress[2]*(Dxi*Dyj + Dxj*Dyi);
	mat[2*i+1 + (2*j+1)*NUM_VARIABLES] += h*stress[1]*Dyi*Dyj;
      }
    }
  }
}
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getRes( TacsScalar * res, 
				       const TacsScalar vars[], 
				       const TacsScalar Xpts[] ){
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getMat( TacsScalar *mat, 
				       TacsScalar *res, 
				       const TacsScalar vars[], 
				       const TacsScalar Xpts[],
				       MatrixOrientation matOr ){
  ...
  for ( int n = 0; n < numGauss; n++ ){
    ...
    addGeoStiffness(mat, h, stress, J, Na, Nb);
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getResXptSens( TacsScalar * res, 
					      const TacsScalar vars[],
					      const TacsScalar Xpts[] ){
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getResDVSens( int dvNum, 
					     TacsScalar * res, 
					     const TacsScalar vars[], 
					     const TacsScalar Xpts[] ){
  memset(res, 0, NUM_VARIABLES*sizeof(TacsScalar));
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::addAdjResDVSensProduct( TacsScalar alpha,
						       TacsScalar dvSens[], 
						       int dvLen,
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getMatType( ElementMatrixTypes matType, 
  ...
  if (matType == MASS_MATRIX){  
	...
	for ( int i = 0; i <= j; i++ ){
	  ...
	  mat[2*i+1 + 2*j*NUM_VARIABLES] += d;

	  mat[2*i + (2*j+1)*NUM_VARIABLES] += d;
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getMatTypeDVSens( int dvNum, 
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getPtwiseStrain( TacsScalar strain[], 
						const double pt[],
						const TacsScalar vars[], 
						const TacsScalar Xpts[] ){
  // The shape functions associated with the element
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::addPtwiseStrainSVSens( TacsScalar sens[], 
						      const double pt[], 
						      const TacsScalar scaleFactor, 
						      const TacsScalar strainSens[],
						      const TacsScalar vars[], 
...
*/
template <int NUM_NODES>
void TACS2DElement<NUM_NODES>::getPtwiseStrainXptSens( TacsScalar strain[], 
