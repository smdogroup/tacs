// Op-level second-order verification for A2D::ADMatTrans3x3ADMatMult
...
// TestADMatTrans3x3ADMatMult, scale=1 case), generalized here to also cover
// the scaled constructor (mirrors MatTrans3x3ADMatMult's own
// TacsRealPart(scale)==1.0 branch convention).
//
// clang-format off
// Build (real mode):
//   mpicxx -std=c++11 -I<repo>/src -I<repo>/src/elements/a2d \
//     test_admattrans3x3admatmult_second_order.cpp -o t21_real && ./t21_real
// Build (complex mode):
//   mpicxx -std=c++11 -DTACS_USE_COMPLEX -I<repo>/src -I<repo>/src/elements/a2d \
//     test_admattrans3x3admatmult_second_order.cpp -o t21_complex && ./t21_complex
// clang-format on
//
// C = scale*A^T*B is bilinear in its two active inputs (A, B), so both
// hforward and hreverse carry cross terms between A and B.
...
int main() {
  ...
  // --- (1) hforward vs directional derivative of the primal ------------------
  {
    ...
    ADMat3x3 Ct;
    ADMatTrans3x3ADMatMult op(scale, At, Bt, Ct);
    op.hforward();
    ...
    ADMat3x3 Ch;
    ADMatTrans3x3ADMatMult(scale, Ah, Bh, Ch);
    double maxerr = 0.0;
    for (int i = 0; i < 9; i++) {
    ...
    ADMat3x3 Cph, Cmh;
    ADMatTrans3x3ADMatMult(scale, Aph, Bph, Cph);
    ADMatTrans3x3ADMatMult(scale, Amh, Bmh, Cmh);
    double maxerr = 0.0;
    for (int i = 0; i < 9; i++) {
...
#endif
    printf(
        "  [ADMatTrans3x3ADMatMult] hforward vs "
  ...
  // --- (2) hreverse (.Ah) vs hand-computed formula ---------------------------
  {
    ...
    // Expected (from TestADMatTrans3x3ADMatMult, E7):
    //  A.Ah = scale*(B.A*Ch^T + Bp*Cd^T)
    //  B.Ah = scale*(A.A*Ch  + Ap*Cd)
    double expectedAh[9] = {0}, expectedBh[9] = {0};
    for (int i = 0; i < 3; i++) {
      for (int k = 0; k < 3; k++) {
        double sA = 0.0, sB = 0.0;
        for (int j = 0; j < 3; j++) {
          sA += TacsRealPart(B.A[3 * i + j]) * TacsRealPart(Ch[3 * k + j]);
          sA += TacsRealPart(Bp[3 * i + j]) * TacsRealPart(Cd[3 * k + j]);
          sB += TacsRealPart(A.A[3 * i + j]) * TacsRealPart(Ch[3 * j + k]);
          sB += TacsRealPart(Ap[3 * i + j]) * TacsRealPart(Cd[3 * j + k]);
        }
    ...
    printf(
        "  [ADMatTrans3x3ADMatMult] hreverse (.Ah) vs hand-computed formula: "
        "%.3e\n",
        maxerr);
  ...
  // --- (3) First-order forward()/reverse() call sites are unaffected --------
  {
    ...
    ADMatTrans3x3ADMatMult op0(A0, B0, C0);
    op0.forward();
    op0.reverse();
  }
