...
/*
  ...
  Copyright (c) 2013-2016 Graeme Kennedy. All rights reserved. 
*/
...
*/
void BCSCMat::multAdd( TacsScalar * X, TacsScalar * Z, 
                       TacsScalar * Y, int vec_bsize ){
  if (Y != Z){
...
void BCSCMat::getArrays( int *_nrows, int *_ncols, int *_nblock_cols, 
  ...
  if (_nblock_cols){ *_nblock_cols = nblock_cols; }
  if (_bptr){ *_bptr = bptr; }
  if (_aptr){ *_aptr = aptr; }
  if (_colp){ *_colp = colp; }
  if (_rows){ *_rows = rows; }
...
BCSCMatPivot::~BCSCMatPivot(){
  ...
  if (temp_array){ delete [] temp_array; }
  if (temp_iarray){ delete [] temp_iarray; }
  if (temp_column){ delete [] temp_column; }
  if (perm){ delete [] perm; }
  if (var_to_node){ delete [] var_to_node; }
  if (node_to_vars_ptr){ delete [] node_to_vars_ptr; }
  if (node_to_vars){ delete [] node_to_vars; }
  if (LU){ delete [] LU; }
  if (lu_diag_ptr){ delete [] lu_diag_ptr; }
  if (lu_col_ptr){ delete [] lu_col_ptr; }
  if (lu_rows){ delete [] lu_rows; }
}
...
void BCSCMatPivot::applyNodeUpdate( int node,
  ...
  BLAStrsm("R", "U", "N", "U",
           &spa_dim, &node_dim, &alpha, L, &node_dim,
           temp_block, &spa_dim);
  ...
  for ( int j = lu_diag_ptr[node] + node_dim; j < j_end; 
    ...
    TacsScalar alpha = 1.0, beta = 0.0;
    BLASgemm("N", "N", &spa_dim, &col_size, &node_dim,
             &alpha, temp_block, &spa_dim,
             L, &node_dim, &beta, temp_cols, &spa_dim);
...
void BCSCMatPivot::applyNodeUpperUpdate( int node,
  ...
  TacsScalar alpha = 1.0;
  BLAStrsm("R", "L", "N", "N",
           &spa_dim, &node_dim, &alpha, U, &node_dim,
           temp_block, &spa_dim);
  ...
  for ( int j = lu_col_ptr[node]; j < j_end; j += max_col_size ){
    ...
    BLASgemm("N", "N", &spa_dim, &col_size, &node_dim, 
             &alpha, temp_block, &spa_dim,
             U, &node_dim, &beta, temp_cols, &spa_dim);
...
*/
void BCSCMatPivot::factorNode( int node, 
                               TacsScalar *col, int node_dim,
                               int *rows, int num_rows, 
                               int diag_index ){
  ...
  for ( int j = 0; j < node_dim; diag_index++, j++ ){
    ...
    for ( int i = diag_index+1; i < num_rows; i++ ){
      TacsScalar abs_val = fabs(c[0]);
      if (abs_val > fabs(max_entry)){
	max_row_index = i;
	max_entry = c[0];
      }
      c += node_dim;
    ...
    if (fabs(max_entry) > fabs(diag_entry)){
...
double BCSCMatPivot::factor( double _fill ){
  ...
  for ( int node = 0; node < nblock_cols; node++ ){
    ...
    for ( int i = num_nodes-1; i >= 0; i-- ){
      ...
      applyNodeUpdate(node, cdim, column, node_dim,
                      temp_block, node_dim*node_dim,
                      temp_cols, temp_array_size);
    }
    ...
    for ( int i = num_nodes-1; i >= 0; i-- ){
      int node = topo_order[i];
      for ( int k = node_to_vars_ptr[node]; 
            k < node_to_vars_ptr[node+1]; k++ ){
    ...
    factorNode(node, &LU[lu_aptr[node]], node_dim,
               &lu_rows[col_ptr], num_rows, diag_index);
  }
...
/*
  ...
  B = L^{-1}*B
*/
void BCSCMatPivot::applyLower( TacsScalar *B, int vec_bsize,
                               TacsScalar *temp, int temp_size ){
  ...
  for ( int node = 0; node < nblock_cols; node++ ){
    int node_dim = bptr[node+1] - bptr[node];
    // Compute L^{-1}*B
    applyNodeUpdate(node, node_dim, B, vec_bsize,
                    temp_block, temp_block_size,
...
/*
  ...
  B = U^{-1}*B
*/
void BCSCMatPivot::applyUpper( TacsScalar *B, int vec_bsize,
                               TacsScalar *temp, int temp_size ){
  TacsScalar *temp_block = &temp[0];
  TacsScalar *temp_cols = &temp[vec_bsize*max_block_size];
  ...
  for ( int node = nblock_cols-1; node >= 0; node-- ){
    int node_dim = bptr[node+1] - bptr[node];
    // Compute U^{-1}*B
    applyNodeUpperUpdate(node, node_dim, B, vec_bsize,
                         temp_block, temp_block_size,
                         temp_cols, temp_cols_size);
  }
}
