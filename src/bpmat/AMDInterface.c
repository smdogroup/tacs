...
static void amd_add_space( int nvars, int * rowp, int * cols, 
			   const int * alen, const int r, int required_space ){
...
/*
  ...
  Reordering Algorithm, SIAM J. Mat. Anl. 1996), but without
  supernodes (supervariables). Their ordering code - called AMD -
  works better as a result. The advantage of the approach below, is
  that the ordering of the interface unknowns, used in the global
  Schur complement system, can be delayed until all other variables
  are ordered. This allows a the full Schur method to be used
  efficiently within TACS.
  ...
  slen: The length of the supernode! If negative, it points to the
...
void amd_order_interface( int nvars, int * rowp, int * cols, int * perm,
  ...
  int * degree = new int[nvars]; // The degree (exact or estimate) of the node
...
variables, not element %d\n", piv, cols[j]);
	...
	for ( int k = j+1; k < lenlp; k++ ){
      ...
      for ( int j = 0; j < lenlp; j++ ){
	int lj = Lp[j];
	if (lj == piv || slen[lj] < 0){ continue; }
	
	for ( int k = j+1; k < lenlp; k++ ){
	  int lk = Lp[k];
	  if (lk == piv || slen[lk] < 0){ continue; }
