...
class BCSCMat : public TACSObject {
 public:
  ...
  BCSCMat( MPI_Comm _comm, int bsize, int _nrows, int _ncols,
           int **_colp, int **_rows, TacsScalar **_A = NULL );

  // Take in a CSR-type format and use it to generate the CSC data
  BCSCMat( MPI_Comm _comm, int bsize,
  ...
  BCSCMat( MPI_Comm _comm, const int bsize[],
           int num_block_rows, int num_block_cols,
           const int *block_rowp, const int *block_cols );
  ...
  // ----------------------------
  void zeroEntries();
  void addMatBlockValues( int num_rows, const int brows[],
  ...
  void multAdd( TacsScalar *X, TacsScalar *Z,
                TacsScalar *Y, int vec_bsize );

  // Retrieve the arrays from the underlying data structure
  // ------------------------------------------------------
  int getMaxBlockSize();
  void getArrays( int *_nrows, int *_ncols, int *_nblock_cols,
...
class BCSCMatPivot : public TACSObject {
 ...
 private:
  ...
  void applyLower( TacsScalar *B, int vec_bsize,
                   TacsScalar *temp, int temp_size );

  // Apply the upper portion of the factorization
  void applyUpper( TacsScalar *B, int vec_bsize,
                   TacsScalar *temp, int temp_size );

  // Apply the column update, spa = L[node]^{-1}*spa, to the columns
  void applyNodeUpdate( int node, int node_dim,
                        TacsScalar *spa, int spa_width,
  ...
  void applyNodeUpperUpdate( int node, int node_dim,
                             TacsScalar *spa, int spa_width,
                             TacsScalar *temp_block, int temp_block_size,
                             TacsScalar *temp_cols, int temp_cols_size );

  // Factor the panel matrix associated with a node
  void factorNode( int node, TacsScalar *column, int node_size,
