...
EPGeneralizedShiftInvert::EPGeneralizedShiftInvert( TacsScalar _sigma, 
  ...
  temp = inner->createVec(); temp->incref();
}
...
*/
void EPGeneralizedShiftInvert::setSigma( TacsScalar _sigma ){ 
  sigma = _sigma; 
}
...
void EPBucklingShiftInvert::setSigma( TacsScalar _sigma ){ 
  sigma = _sigma; 
}
...
*/
void SEP::setOrthoType( enum OrthoType _ortho_type ){
  ortho_type = _ortho_type;
}
...
void SEP::solve( KSMPrint *ksm_print, KSMPrint *ksm_file ){
  ...
  if (ksm_print){
    ...
    sprintf(line, "%3s %18s %18s %10s\n",
	    " ", "eigenvalue", "shift-invert eig", "error");
...
void SEP::sortEigenvalues( TacsScalar *values, int neigs, int *p ){
  ...
  int use_abs = (spectrum == SMALLEST_MAGNITUDE || 
                 spectrum == LARGEST_MAGNITUDE);
  int sort_ascending = (spectrum == SMALLEST_MAGNITUDE ||
                        spectrum == SMALLEST);
