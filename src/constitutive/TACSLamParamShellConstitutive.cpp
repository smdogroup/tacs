...
const char *TACSLamParamShellConstitutive::constName = "TACSLamParamShellConstitutive";

TACSLamParamShellConstitutive::TACSLamParamShellConstitutive( TACSOrthotropicPly * _orthoPly,
...
int TACSLamParamShellConstitutive::getDesignVarNums( int elemIndex,
                                                     int dvLen,
                                                     int dvNums[] ){
  if (dvNums && dvLen >= numDesignVars){
    int i = 0;
    if (tNum >= 0){ dvNums[i] = tNum;  i++; }
    if (n0 >= 0){   dvNums[i] = n0;    i++; }
    if (n45 >= 0){  dvNums[i] = n45;   i++; }
    if (n90 >= 0){  dvNums[i] = n90;   i++; }
    if (nW1 >= 0){  dvNums[i] = nW1;   i++; }
    if (nW3 >= 0){  dvNums[i] = nW3;   i++; }
  }
  return numDesignVars;
}

int TACSLamParamShellConstitutive::setDesignVars( int elemIndex,
                                                  int dvLen,
                                                  const TacsScalar dvs[] ){
  int i = 0;
  if (tNum >= 0){ t = dvs[i];   i++; }
  if (n0 >= 0){   f0 = dvs[i];  i++; }
  if (n45 >= 0){  f45 = dvs[i]; i++; }
  if (n90 >= 0){  f90 = dvs[i]; i++; }
  if (nW1 >= 0){  W1 = dvs[i];  i++; }
  if (nW3 >= 0){  W3 = dvs[i];  i++; }
  return numDesignVars;
}

int TACSLamParamShellConstitutive::getDesignVars( int elemIndex,
                                                  int dvLen,
                                                  TacsScalar dvs[] ){
  int i = 0;
  if (tNum >= 0){ dvs[i] = t;    i++; }
  if (n0 >= 0){   dvs[i] = f0;   i++; }
  if (n45 >= 0){  dvs[i] = f45;  i++; }
  if (n90 >= 0){  dvs[i] = f90;  i++; }
  if (nW1 >= 0){  dvs[i] = W1;   i++; }
  if (nW3 >= 0){  dvs[i] = W3;   i++; }
  return numDesignVars;
}

int TACSLamParamShellConstitutive::getDesignVarRange( int elemIndex,
                                                      int dvLen,
                                                      TacsScalar lb[],
                                                      TacsScalar ub[] ){
  int i = 0;
  ...
  if (n0 >= 0){
    lb[i] = min_f0;   ub[i] = 1.0;  i++;
  }
  if (n45 >= 0){
    lb[i] = min_f45;  ub[i] = 1.0;  i++;
  }
  if (n90 >= 0){
    lb[i] = min_f90;  ub[i] = 1.0;  i++;
  }
  if (nW1 >= 0){
    lb[i] = -1.0;     ub[i] = 1.0;  i++;
  }
...
void TACSLamParamShellConstitutive::addDensityDVSens( int elemIndex,
                                                      TacsScalar scale,
                                                      const double pt[],
                                                      const TacsScalar X[],
                                                      int dvLen,
...
void TACSLamParamShellConstitutive::getStiffness( TacsScalar A[],
                                                  TacsScalar B[],
                                                  TacsScalar D[],
                                                  TacsScalar As[],
                                                  TacsScalar *drill ){
  ...
  if (!checkDeterminant(A)){
    fprintf(stderr, "TACSLamParamShellConstitutive: Error, A has negative eigenvalues\n");
  }

  if (!checkDeterminant(D)){
    fprintf(stderr, "TACSLamParamShellConstitutive: Error, D has negative eigenvalues\n");
    fprintf(stderr, "W = [%12.6e, %12.6e, %12.6e %12.6e]\n",
            TacsRealPart(W1), 0.0, TacsRealPart(W3), 0.0);
  }
...
// Evaluate the stress
void TACSLamParamShellConstitutive::evalStress( int elemIndex,
                                                const double pt[],
                                                const TacsScalar X[],
                                                const TacsScalar e[],
                                                TacsScalar s[] ){
...
void TACSLamParamShellConstitutive::addStressDVSens( int elemIndex,
                                                     TacsScalar scale,
                                                     const double pt[],
                                                     const TacsScalar X[],
                                                     const TacsScalar e[],
  ...
  if (tNum >= 0){
    ...
    dfdx[0] += scale*(sAs0*psi[6]*e[6] + sAs2*psi[7]*e[7] +
...
*/
void TACSLamParamShellConstitutive::computeFailureStrainSens( const TacsScalar strain[],
...
*/
TacsScalar TACSLamParamShellConstitutive::evalFailure( int elemIndex,
                                                       const double pt[],
                                                       const TacsScalar X[],
...
/**
  ...
  values
*/
TacsScalar TACSLamParamShellConstitutive::evalFailureStrainSens( int elemIndex,
                                                                 const double pt[],
                                                                 const TacsScalar X[],
                                                                 const TacsScalar strain[],
                                                                 TacsScalar sens[] ){
...
*/
void TACSLamParamShellConstitutive::addFailureDVSens( int elemIndex,
                                                      TacsScalar scale,
                                                      const double pt[],
                                                      const TacsScalar X[],
                                                      const TacsScalar strain[],
  ...
  if (tNum >= 0){
    for ( int k = 0; k < NUM_FAIL_ANGLES; k++ ){
      ...
      dfdx[index] += 0.5*factor*weights[2*k]*(strain[3]*e_sens[0] +
                                              strain[4]*e_sens[1] +
                                              strain[5]*e_sens[2]);
      ...
      dfdx[index] -= 0.5*factor*weights[2*k+1]*(strain[3]*e_sens[0] +
                                                strain[4]*e_sens[1] +
                                                strain[5]*e_sens[2]);
    }
...
TacsScalar TACSLamParamShellConstitutive::evalDesignFieldValue( int elemIndex,
                                                                const double pt[],
                                                                const TacsScalar X[],
                                                                int index ){
  if (index == 0){
