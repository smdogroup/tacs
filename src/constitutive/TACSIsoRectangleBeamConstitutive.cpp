...
TACSIsoRectangleBeamConstitutive::TACSIsoRectangleBeamConstitutive( TACSMaterialProperties *properties,
                                                                    TacsScalar _width,
                                                                    TacsScalar _thickness,
                                                                    int _width_num,
                                                                    int _thickness_num,
                                                                    TacsScalar _lb_width,
                                                                    TacsScalar _ub_width,
...
int TACSIsoRectangleBeamConstitutive::getDesignVarNums( int elemIndex,
                                                        int dvLen,
                                                        int dvNums[] ){
  int index = 0;
  if (width_num >= 0){
...
int TACSIsoRectangleBeamConstitutive::setDesignVars( int elemIndex,
                                                     int dvLen,
...
int TACSIsoRectangleBeamConstitutive::getDesignVars( int elemIndex,
                                                     int dvLen,
                                                     TacsScalar dvs[] ){
  int index = 0;
  if (width_num >= 0){
...
int TACSIsoRectangleBeamConstitutive::getDesignVarRange( int elemIndex, int dvLen,
                                                         TacsScalar lb[],
                                                         TacsScalar ub[] ){
  int index = 0;
  if (width_num >= 0){
...
void TACSIsoRectangleBeamConstitutive::addMassMomentsDVSens( int elemIndex,
                                                             const double pt[],
                                                             const TacsScalar X[],
...
TacsScalar TACSIsoRectangleBeamConstitutive::evalSpecificHeat( int elemIndex,
                                                               const double pt[],
                                                               const TacsScalar X[] ){
  if (props){
...
void TACSIsoRectangleBeamConstitutive::addDensityDVSens( int elemIndex,
                                                         TacsScalar scale,
                                                         const double pt[],
                                                         const TacsScalar X[],
                                                         int dvLen,
...
void TACSIsoRectangleBeamConstitutive::evalTangentStiffness( int elemIndex,
                                                             const double pt[],
                                                             const TacsScalar X[],
                                                             TacsScalar C[] ){
  TacsScalar E, nu;
...
void TACSIsoRectangleBeamConstitutive::addStressDVSens( int elemIndex,
                                                        TacsScalar scale,
                                                        const double pt[],
                                                        const TacsScalar X[],
                                                        const TacsScalar e[],
  ...
  if (width_num >= 0){
    ...
    TacsScalar dJ = da * b3 * (16.0 / 3.0 - 3.36 * b / a * (1.0 - b4 / a4 / 12.0)) +
  ...
  if (thickness_num >= 0){
    ...
    TacsScalar a4 = a * a * a * a;
    TacsScalar dJ = a * db3 * (16.0 / 3.0 - 3.36 * b / a * (1.0 - b4 / a4 / 12.0)) +
                    a * b3 * (-3.36 * db / a * (1.0 - b4 / a4 / 12.0)) + a * b3 * (-3.36 * b / a * (-db4 / a4 / 12.0));
...
TacsScalar TACSIsoRectangleBeamConstitutive::evalFailureStrainSens( int elemIndex,
                                                                    const double pt[],
                                                                    const TacsScalar X[],
                                                                    const TacsScalar e[],
                                                                    TacsScalar sens[] ){
  ...
  for (int i = 0; i < count; i++) {
    for (int k = 0; k < NUM_STRESSES; k++) {
      sens[k] += exp(ks_weight * (fail_checks[i] - max_fail)) * fail_checks_sens[i][k] / ks_sum;
    }
  }
...
void TACSIsoRectangleBeamConstitutive::addFailureDVSens( int elemIndex,
                                                         TacsScalar scale,
                                                         const double pt[],
                                                         const TacsScalar X[],
                                                         const TacsScalar e[],
...
TacsScalar TACSIsoRectangleBeamConstitutive::evalDesignFieldValue( int elemIndex,
                                                                   const double pt[],
                                                                   const TacsScalar X[],
                                                                   int index ){
  if (index == 0){
...
const char* TACSIsoRectangleBeamConstitutive::constName = "TACSIsoRectangleBeamConstitutive";
