...
int TacsTestConstitutiveDensity( TACSConstitutive *con,
                                 int elemIndex,
                                 const double pt[],
                                 const TacsScalar X[],
                                 int ndvs,
  ...
  if (test_print_level > 0){
    fprintf(stderr,
            "Testing the derivative of the density for %s\n",
            con->getObjectName());
  ...
  if (test_print_level > 1){
    TacsPrintErrorComponents(stderr, "d(rho)/dx",
...
int TacsTestConstitutiveSpecificHeat( TACSConstitutive *con,
                                      int elemIndex,
                                      const double pt[],
                                      const TacsScalar X[],
                                      int ndvs,
  ...
  if (test_print_level > 0){
    fprintf(stderr,
            "Testing the derivative of the specific heat for %s\n",
            con->getObjectName());
    fprintf(stderr, "Max Err: %10.4e in component %d.\n",
            max_err, max_err_index);
    fprintf(stderr, "Max REr: %10.4e in component %d.\n",
  ...
  // Print the error if required
  if (test_print_level > 1){
    TacsPrintErrorComponents(stderr, "d(c)/dx",
                             dfdx, dfdx_approx, ndvs);
  }
  if (test_print_level){ fprintf(stderr, "\n"); }
...
int TacsTestConstitutiveStress( TACSConstitutive *con,
  ...
  if (test_print_level > 0){
    fprintf(stderr,
            "Testing the derivative of the stress-adjoint product %s\n",
            con->getObjectName());
    fprintf(stderr, "Max Err: %10.4e in component %d.\n",
            max_err, max_err_index);
    fprintf(stderr, "Max REr: %10.4e in component %d.\n",
  ...
  // Print the error if required
  if (test_print_level > 1){
    TacsPrintErrorComponents(stderr, "d(psi^{T}s)/dx",
                             dfdx, dfdx_approx, ndvs);
  }
  if (test_print_level){ fprintf(stderr, "\n"); }
...
int TacsTestConstitutiveThermalStrain( TACSConstitutive *con,
                                       int elemIndex,
                                       const double pt[],
                                       const TacsScalar X[],
                                       int ndvs,
  ...
  // Print the error if required
  if (test_print_level > 1){
    TacsPrintErrorComponents(stderr, "d(psi^{T}et)/dx",
                             dfdx, dfdx_approx, ndvs);
  }
  if (test_print_level){ fprintf(stderr, "\n"); }
...
int TacsTestConstitutiveFailure( TACSConstitutive *con,
  ...
  // Print the error if required
  if (test_print_level > 1){
    TacsPrintErrorComponents(stderr, "d(failure)/dx",
                             dfdx, dfdx_approx, ndvs);
  }
  if (test_print_level){ fprintf(stderr, "\n"); }
...
int TacsTestConstitutive( TACSConstitutive *con,
  ...
  int flag, fail = 0;
  flag = TacsTestConstitutiveDensity(con, elemIndex, pt, X,
  ...
  flag = TacsTestConstitutiveThermalStrain(con, elemIndex, pt, X,
                                           ndvs, dvs, dh, test_print_level,
                                           test_fail_atol, test_fail_rtol);
  fail = flag || fail;
