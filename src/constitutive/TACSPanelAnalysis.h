...
class TACSPanelAnalysis : public TACSObject {
 public:
  ...
  void setSegment( int seg, int seg_type, FSDTStiffness *stiff,
                   int n1, int n2 );
  ...
  // -------------------------------------------------
  void setFailurePoints( int _npoints, int segments[], int nodes[], int is_skin[] );
  void failure( const TacsScalar strain[],
 ...
 private:
  ...
  void addStiffMatBeam( TacsScalar mat[], int n1,
                        const TacsScalar Ct[] );
  ...
  // -----------------------------------------------
  void addGeoStiffMat( TacsScalar mat[],
                       const TacsScalar s[], int n1, int n2 );

  // Add the material-dependent sensitivity from a segment
  // -----------------------------------------------------
  void addMassMatDVSens( TacsScalar smat[],
  ...
  // ------------------------------------------------------
  void addMassMatGeoSens( TacsScalar smat[], int dv,
                          const TacsScalar Z[], int ldz, int nz,
                          int n1, int n2, const TacsScalar mass[] );
  ...
  // -----------------------------------------------
  void addMassMatLxSens( TacsScalar smat[],
                         const TacsScalar Z[], int ldz, int nz,
                         int n1, int n2, const TacsScalar mass[] );
  ...
  void addValuesTranspose( TacsScalar mat[], int kband,
                           int n, const int rows[],
                           int m, const int cols[],
                           const TacsScalar values[] );

  void addSensMatValues( TacsScalar smat[], const TacsScalar Z[],
                         int ldz, int nz,
                         int n, const int rows[],
                         int m, const int cols[],
                         const TacsScalar values[] );
  ...
  // ---------------------------------------------------------------
  void transformVec( TacsScalar vec[], const TacsScalar t[],
                     int nn );
  void transformMat( TacsScalar mat[], const TacsScalar t[],
                     int nn, int mm );
  void transformMatGeoSens( TacsScalar mat[], TacsScalar smat[],
                            const TacsScalar t[], const TacsScalar st[],
                            int nn, int mm );

  // Code to compute the derivative of the strain w.r.t. the nodal displacements
  // ---------------------------------------------------------------------------
  void computeBsin( TacsScalar B[], TacsScalar lambda,
                     TacsScalar h, const double N[], const double Na[],
                     const double Nhp[], const double Nahp[],
                     const double Naahp[] );
  void computeBcos( TacsScalar B[], TacsScalar lambda,
                    TacsScalar h, const double N[], const double Na[],
                    const double Nhp[], const double Nahp[],
                    const double Naahp[] );

  void computeBsinGeoSens( TacsScalar B[], TacsScalar lambda,
                           TacsScalar slambda, TacsScalar h, TacsScalar sh,
                           const double N[], const double Na[],
                           const double Nhp[], const double Nahp[],
                           const double Naahp[] );
  void computeBcosGeoSens( TacsScalar B[], TacsScalar lambda,
                           TacsScalar slambda, TacsScalar h, TacsScalar sh,
                           const double N[], const double Na[],
                           const double Nhp[], const double Nahp[],
                           const double Naahp[] );
  ...
  void computeMsinGeoSens( TacsScalar M[], TacsScalar lambda,
                           ...
                           const double N[], const double Nhp[],
                           const double Nahp[] );
  void computeMcosGeoSens( TacsScalar M[], TacsScalar lambda,
                           TacsScalar slambda, TacsScalar h, TacsScalar sh,
                           const double N[], const double Nhp[],
                           const double Nahp[] );
  ...
  TacsScalar innerBeamProduct( const TacsScalar s[],
                               const TacsScalar e[] ){
  ...
  void computeStress( TacsScalar s[], const TacsScalar e[],
    ...
    s[1] = At[1]*e[0]+At[3]*e[1]+At[4]*e[2] + Bt[1]*e[3]+Bt[3]*e[4]+Bt[4]*e[5];
    s[2] = At[2]*e[0]+At[4]*e[1]+At[5]*e[2] + Bt[2]*e[3]+Bt[4]*e[4]+Bt[5]*e[5];

    s[3] = Bt[0]*e[0]+Bt[1]*e[1]+Bt[2]*e[2] + Dt[0]*e[3]+Dt[1]*e[4]+Dt[2]*e[5];
    s[4] = Bt[1]*e[0]+Bt[3]*e[1]+Bt[4]*e[2] + Dt[1]*e[3]+Dt[3]*e[4]+Dt[4]*e[5];
