...
*/
bladeFSDTStiffness::bladeFSDTStiffness( OrthoPly * _ortho_ply,
                                        TacsScalar _kcorr,
  ...
  num_pf_con = 0;
  if (num_pf_unique > 0){ num_pf_con++; }
  if (num_stiff_pf_unique > 0){ num_pf_con++; }
...
bladeFSDTStiffness::~bladeFSDTStiffness(){
  ortho_ply->decref();
}
...
TacsScalar bladeFSDTStiffness::getFailureCriterion( const TacsScalar stress[8] ){
  ...
  C[0] = A[0];  C[8]  = A[1];  C[16] = A[2];
  C[1] = A[1];  C[9]  = A[3];  C[17] = A[4];
  C[2] = A[2];  C[10] = A[4];  C[18] = A[5];

  C[3] = B[0];  C[11] = B[1];  C[19] = B[2];
  C[4] = B[1];  C[12] = B[3];  C[20] = B[4];
  C[5] = B[2];  C[13] = B[4];  C[21] = B[5];

  // Copy the B/D matrices to C
  C[24] = B[0];  C[32] = B[1];  C[40] = B[2];
  C[25] = B[1];  C[33] = B[3];  C[41] = B[4];
  C[26] = B[2];  C[34] = B[4];  C[42] = B[5];

  C[27] = D[0];  C[35] = D[1];  C[43] = D[2];
  C[28] = D[1];  C[36] = D[3];  C[44] = D[4];
  C[29] = D[2];  C[37] = D[4];  C[45] = D[5];
...
TacsScalar bladeFSDTStiffness::getBucklingCriterion( const TacsScalar stress[8] ){
  TacsScalar strain[8];
  memcpy(strain, stress, 8*sizeof(TacsScalar));
...
/*
  ...
  constraints.
*/
int bladeFSDTStiffness::isLinear(){
  return 1;
}

// The ply fractions must add up to one
int bladeFSDTStiffness::getNumCon(){
  return num_pf_con;
}
...
int bladeFSDTStiffness::getConRange( int offset,
...
int bladeFSDTStiffness::ownsDesignVar( const int dvNum ) const {
  if (Lx_num == dvNum){ return 1; }
  else if (sp_num == dvNum){ return 1; }
  else if (sh_num == dvNum){ return 1; }
  else if (st_num == dvNum){ return 1; }
  else if (t_num == dvNum){ return 1; }
...
void bladeFSDTStiffness::setDesignVars( const TacsScalar dvs[], int numDVs ){
  if (Lx_num >= 0 && Lx_num < numDVs){ Lx = dvs[Lx_num]; }
  if (sp_num < numDVs){ sp = dvs[sp_num]; }
  if (sh_num < numDVs){ sh = dvs[sh_num]; }
  if (st_num < numDVs){ st = dvs[st_num]; }
  if (t_num < numDVs){ t = dvs[t_num]; }
...
void bladeFSDTStiffness::updateStiffness(){
  ...
  TacsScalar Es = (Qbar_stiff[0] -
                   (Qbar_stiff[1]*Qbar_stiff[1])/Qbar_stiff[3]);
...
int bladeFSDTStiffness::getLxNum(){
  return Lx_num;
}

void bladeFSDTStiffness::getDesignVars( TacsScalar dvs[], int numDVs ) {
  if (Lx_num >= 0 && Lx_num < numDVs){ dvs[Lx_num] = Lx; }
  if (sp_num >= 0 && sp_num < numDVs){ dvs[sp_num] = sp; }
  if (sh_num >= 0 && sh_num < numDVs){ dvs[sh_num] = sh; }
  if (st_num >= 0 && st_num < numDVs){ dvs[st_num] = st; }
  if (t_num >= 0 && t_num < numDVs){ dvs[t_num] = t; }
...
*/
void bladeFSDTStiffness::pointwiseMassDVSens( int dvNum,
                                              const double gpt[],
  ...
  if (dvNum >= 0){
    if (t_num == dvNum){
      ...
      massDVSens[0] += ortho_ply->getRho()*Area_sens*sfact;
      massDVSens[2] = (3.0/4.0)*ortho_ply->getRho()*st*st*Area_sens*sfact;
    }
  }
}

TacsScalar bladeFSDTStiffness::getDensity(){
  return ortho_ply->getRho();
}
...
*/
TacsScalar bladeFSDTStiffness::getStiffness( const double gpt[],
                                             TacsScalar _A[], TacsScalar _B[],
                                             TacsScalar _D[],
                                             TacsScalar _As[] ){
  for ( int k = 0; k < 6; k++ ){
    _A[k] = A[k];
...
/*
  ...
  elsewhere.
*/
TacsScalar bladeFSDTStiffness::getStiffnessDVSens( int dvNum,
                                                   const double gpt[],
                                                   TacsScalar sA[],
                                                   TacsScalar sB[],
                                                   TacsScalar sD[],
  ...
  if (dvNum >= 0 && ownsDesignVar(dvNum)){
    if (t_num == dvNum){
      ...
      sD[0] += Es*(Area_sens*es*es + 2.0*Area*es*es_sens + Is_sens)*sfact;
      ...
      TacsScalar Es_sens =
        (sQbar_stiff[0] - 2.0*(Qbar_stiff[1]*sQbar_stiff[1])/Qbar_stiff[3] +
         sQbar_stiff[3]*(Qbar_stiff[1]*Qbar_stiff[1])/(Qbar_stiff[3]*Qbar_stiff[3]));
...
TacsScalar bladeFSDTStiffness::calculateStressDVSens( const double pt[],
  ...
  s_sens[0] = A_sens[0]*strain[0] + A_sens[1]*strain[1] + A_sens[2]*strain[2] +
         B_sens[0]*strain[3] + B_sens[1]*strain[4] + B_sens[2]*strain[5];
  s_sens[1] = A_sens[1]*strain[0] + A_sens[3]*strain[1] + A_sens[4]*strain[2] +
         B_sens[1]*strain[3] + B_sens[3]*strain[4] + B_sens[4]*strain[5];
  s_sens[2] = A_sens[2]*strain[0] + A_sens[4]*strain[1] + A_sens[5]*strain[2] +
         B_sens[2]*strain[3] + B_sens[4]*strain[4] + B_sens[5]*strain[5];

  // Compute the bending moments
  s_sens[3] = B_sens[0]*strain[0] + B_sens[1]*strain[1] + B_sens[2]*strain[2] +
         D_sens[0]*strain[3] + D_sens[1]*strain[4] + D_sens[2]*strain[5];
  s_sens[4] = B_sens[1]*strain[0] + B_sens[3]*strain[1] + B_sens[4]*strain[2] +
         D_sens[1]*strain[3] + D_sens[3]*strain[4] + D_sens[4]*strain[5];
  s_sens[5] = B_sens[2]*strain[0] + B_sens[4]*strain[1] + B_sens[5]*strain[2] +
         D_sens[2]*strain[3] + D_sens[4]*strain[4] + D_sens[5]*strain[5];
...
TacsScalar bladeFSDTStiffness::calcCriticalShearLoad( TacsScalar D1,
  ...
  double ks = 50.0;
  TacsScalar Nxy_crit_1 = (4.0/(L*L))*sqrt(D3*D1*xi)*(8.125 + 5.045/xi);
  TacsScalar Nxy_crit_2 = (4.0/(L*L))*sqrt(D1*D3)*(11.7 + 0.532*xi + 0.938*xi*xi);
...
TacsScalar bladeFSDTStiffness::calcCriticalShearLoadSens( TacsScalar D1,
  ...
  TacsScalar Nxy_crit_1_sens = (4.0/(L*L))*(sy*cb - 5.045*sxi*y/(xi*xi));
  if (sL != 0.0){
  ...
  TacsScalar Nxy_crit_2_sens = (4.0/(L*L))*(sy*cb + y*(0.532*sxi + 2.0*0.938*xi*sxi));
  if (sL != 0.0){
...
void bladeFSDTStiffness::calcBucklingProps(){
  ...
  TacsScalar Ep = Qbar_panel[0] - (Qbar_panel[1]*Qbar_panel[1])/Qbar_panel[3];
  TacsScalar Es = Qbar_stiff[0] - (Qbar_stiff[1]*Qbar_stiff[1])/Qbar_stiff[3];
  ...
  TacsScalar D12_stiff =
    ((t+0.5*st)*(t+0.5*st)*(t+0.5*st)*(Qbar_panel[1] + Qbar_stiff[1]))/6.0;
  ...
  TacsScalar D66_stiff =
    ((t+0.5*st)*(t+0.5*st)*(t+0.5*st)*(Qbar_panel[5] + Qbar_stiff[5]))/6.0;
  ...
  Nx_panel_crit = (2.0*M_PI*M_PI/(sp*sp))*(sqrt(D11*D22) + D12 + 2.0*D66);
...
/*
  ...
  sQbar_stiff
*/
void bladeFSDTStiffness::calcBucklingPropsDVSens( int dvNum,
                                                  TacsScalar * sNx_global_crit,
  ...
  if (dvNum == t_num){
    ...
    TacsScalar EI = (zn*zn*(t*sp*Ep + st*sh*sf_fraction*Es) +
                     Es*(st*(sh*sh*sh)/12.0 +
                         st*sh*(zn - 0.5*sh)*(zn - 0.5*sh)));
    TacsScalar EI_sens = (2.0*zn*zn_sens*(t*sp*Ep + st*sh*sf_fraction*Es) +
                          zn*zn*sp*Ep +
    ...
    *sNx_global_crit = (M_PI*M_PI*D1_sens)/(Lx*Lx);
    ...
    TacsScalar D12_stiff_sens =
      ((t+0.5*st)*(t+0.5*st)*(Qbar_panel[1] + Qbar_stiff[1]))/2.0;

    TacsScalar D2_sens =
      (D2*D2/sp)*((sp - sh*sf_fraction)/(D12_panel*D12_panel)*D12_panel_sens +
    ...
    TacsScalar D66_stiff_sens =
      ((t+0.5*st)*(t+0.5*st)*(Qbar_panel[5] + Qbar_stiff[5]))/2.0;

    TacsScalar D3_sens =
      (D3*D3/sp)*((sp - sh*sf_fraction)/(D66_panel*D66_panel)*D66_panel_sens +
    ...
    *sNx_global_crit = (M_PI*M_PI*D1_sens)/(Lx*Lx);
    ...
    TacsScalar D2 = sp/((sp - sh*sf_fraction)/D12_panel +
                        (sh*sf_fraction)/D12_stiff);
    TacsScalar D2_sens = -((D2*D2)/sp)*(sf_fraction/D12_stiff -
                                        sf_fraction/D12_panel);
    ...
    TacsScalar D3 = sp/((sp - sh*sf_fraction)/D66_panel +
                        (sh*sf_fraction)/D66_stiff);
    TacsScalar D3_sens = -((D3*D3)/sp)*(sf_fraction/D66_stiff -
    ...
    *sNx_global_crit = (M_PI*M_PI*D1_sens)/(Lx*Lx);
    ...
    TacsScalar D12_stiff =
      ((t+0.5*st)*(t+0.5*st)*(t+0.5*st)*(Qbar_panel[1] + Qbar_stiff[1]))/6.0;
    TacsScalar D12_stiff_sens =
      1.5*((t+0.5*st)*(t+0.5*st)*(Qbar_panel[1] + Qbar_stiff[1]))/6.0;

    TacsScalar D2 = sp/((sp - sh*sf_fraction)/D12_panel +
                        (sh*sf_fraction)/D12_stiff);
    TacsScalar D2_sens =
      (D2*D2)/sp*((sh*sf_fraction)/(D12_stiff*D12_stiff)*D12_stiff_sens);
    ...
    TacsScalar D66_stiff =
      ((t+0.5*st)*(t+0.5*st)*(t+0.5*st)*(Qbar_panel[5] + Qbar_stiff[5]))/6.0;
    TacsScalar D66_stiff_sens =
      1.5*((t+0.5*st)*(t+0.5*st)*(Qbar_panel[5] + Qbar_stiff[5]))/6.0;
    ...
    *sNx_strong_stiff_crit = -2.0*(M_PI*M_PI*Es*Is_strong)/(Lx*Lx*Lx);
  }
  else if (dvNum >= 0){
    ...
    TacsScalar Ep = Qbar_panel[0] - (Qbar_panel[1]*Qbar_panel[1])/Qbar_panel[3];
    TacsScalar Ep_sens =
      (sQbar_panel[0] - 2.0*(Qbar_panel[1]*sQbar_panel[1])/Qbar_panel[3] +
       sQbar_panel[3]*(Qbar_panel[1]*Qbar_panel[1])/(Qbar_panel[3]*Qbar_panel[3]));
    ...
    // The modulus-weighted area:
    TacsScalar An = (Ep*sp*t + Es*sh*st*(1.0 + sf_fraction));
    TacsScalar An_sens = (Ep_sens*sp*t + Es_sens*sh*st*(1.0 + sf_fraction));
    ...
    TacsScalar EI_sens = (zn*zn*(t*sp*Ep_sens + st*sh*sf_fraction*Es_sens) +
    ...
    *sNx_global_crit = (M_PI*M_PI*D1_sens)/(Lx*Lx);
    ...
    TacsScalar D12_stiff_sens =
      ((t+0.5*st)*(t+0.5*st)*(t+0.5*st)*(sQbar_panel[1] + sQbar_stiff[1]))/6.0;
    TacsScalar D2_sens =
      (D2*D2/sp)*((sp - sh*sf_fraction)/(D12_panel*D12_panel)*D12_panel_sens +
                  (sh*sf_fraction)/(D12_stiff*D12_stiff)*D12_stiff_sens);
    ...
    TacsScalar D66_stiff_sens =
      ((t+0.5*st)*(t+0.5*st)*(t+0.5*st)*(sQbar_panel[5] + sQbar_stiff[5]))/6.0;
...
int bladeFSDTStiffness::calcFail( const TacsScalar strain[], TacsScalar fail[],
  ...
  for ( int k = 0; k < NUM_PLIES; k++ ){
    ...
    if (TacsRealPart(fail[load_num]) > TacsRealPart(max)){ max = fail[load_num]; }
    load_num++;
  }
...
*/
void bladeFSDTStiffness::failure( const double pt[],
                                  const TacsScalar strain[],
                                  TacsScalar * fval ){
...
void bladeFSDTStiffness::calcBucklingStrainSens( const TacsScalar strain[],
  ...
  s_sens[2] = 2.0*weights[load_num]*stress[2]/(Nxy_global_crit*Nxy_global_crit);
...
TacsScalar bladeFSDTStiffness::calcBucklingDVSens( int dvNum,
  ...
  bvalSens += weights[load_num]*
    ((sNx/Nx_global_crit) -
     ...
     2.0*(Nxy*Nxy)/(Nxy_global_crit*Nxy_global_crit*Nxy_global_crit)*sNxy_global_crit);
  load_num++;
  ...
  bvalSens += weights[load_num]*
    (sNx/Nx_panel_crit - (sNx_panel_crit*Nx)/(Nx_panel_crit*Nx_panel_crit) +
     2.0*(Nxy*sNxy)/(Nxy_panel_crit*Nxy_panel_crit) -
     2.0*(sNxy_panel_crit*Nxy*Nxy)/(Nxy_panel_crit*Nxy_panel_crit*Nxy_panel_crit));
  load_num++;

  // Calculate the contribution from the stiffeners
  Nx = -st*sh*(Qbar_stiff[0]*(strain[0] - 0.5*sh*strain[3]) +
                Qbar_stiff[1]*(strain[1] - 0.5*sh*strain[4]));

  sNx = -st*sh*(sQbar_stiff[0]*(strain[0] - 0.5*sh*strain[3]) +
...
/*
  ...
  strain.
*/
void bladeFSDTStiffness::buckling( const TacsScalar strain[],
                                   TacsScalar * bval ){
...
const char * bladeFSDTStiffness::constitutiveName() const {
  return constName;
}
...
*/
int bladeFSDTStiffness::getNumDesignVars(){
    return 5 + 2 * NUM_PLIES;
  }

int bladeFSDTStiffness::getDesignVarNums(int dvNums[], int dvLen){
    if (Lx_num >= 0) {dvNums[0] = Lx_num;}
    else {dvNums[0] = -1;}

    if (sp_num >= 0) {dvNums[1] = sp_num;}
    else {dvNums[1] = -1;}

    if (sh_num >= 0) {dvNums[2] = sh_num;}
    else {dvNums[2] = -1;}

    if (st_num >= 0) {dvNums[3] = st_num;}
    else {dvNums[3] = -1;}

    if (t_num >= 0) {dvNums[4] = t_num;}
    else {dvNums[4] = -1;}

    for (int i = 0; i < NUM_PLIES; i++ ){
       if (pf_nums[i] >= 0) {dvNums[i+5] = pf_nums[i];}
       else {dvNums[i+5] = -1;}

       if (stiff_pf_nums[i] >= 0) {dvNums[i+NUM_PLIES+5] = stiff_pf_nums[i];}
       else {dvNums[i+NUM_PLIES+5] = -1;}
       }
     return 1;
  }
