                                                    ...
                                                    const TacsScalar *Xtrain)
{
  ...
  if (affine)
  {
    relu_term = soft_relu(-Xtest[1] + 0.25 * Xtest[2], theta[0]) * soft_relu(-Xtrain[1] + Xtrain[2], theta[0]);
  }
  else
  {
    relu_term = soft_relu(-Xtest[1], theta[0]) * soft_relu(-Xtrain[1], theta[0]);
  }

  TacsScalar gam_term = theta[1] * Xtest[2] * Xtrain[2];
  TacsScalar sum_sq;
  for (int i = 0; i < 4; i++)
  {
  ...
  return relu_term + gam_term + RQ_term + theta[5];
}
                                                  ...
                                                  TacsScalar *Xtestsens)
{
  ...
  jacobian[1] += soft_relu_sens(-Xtest[1] + 0.25 * Xtest[2], theta[0]) * -1.0 * soft_relu(-Xtrain[1] + Xtrain[2], theta[0]);
  if (affine)
  {
    jacobian[2] += soft_relu_sens(-Xtest[1] + 0.25 * Xtest[2], theta[0]) * 0.25 * soft_relu(-Xtrain[1] + Xtrain[2], theta[0]);
  }
