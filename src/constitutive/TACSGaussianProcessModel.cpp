...
TacsScalar TACSBucklingGaussianProcessModel::kernel(const TacsScalar *Xtest,
  ...
  if (affine) {
    relu_term = soft_relu(-Xtest[1] + 0.25 * Xtest[2], theta[0]) *
                soft_relu(-Xtrain[1] + Xtrain[2], theta[0]);
  } else {
    relu_term =
...
void TACSBucklingGaussianProcessModel::kernelSens(const TacsScalar ksens,
  ...
  if (affine) {
    jacobian[2] += soft_relu_sens(-Xtest[1] + 0.25 * Xtest[2], theta[0]) *
                   0.25 * soft_relu(-Xtrain[1] + Xtrain[2], theta[0]);
  }
...
// TacsScalar SE_kernel = theta[8] * exp(-0.5 * arg1 * arg1 - 0.5 * arg2 *
// arg2); TacsScalar SE_window = soft_relu(theta[11] -
// soft_abs(d_gamma_rho_train, ks), ks) * soft_relu(theta[11] -
// soft_abs(d_gamma_rho_test, ks), ks);
