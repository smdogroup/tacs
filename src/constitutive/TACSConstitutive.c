...
void TACSConstitutive::calculateStressDVSens( int dvNum,
					      const double gpt[], 
					      const TacsScalar strain[], 
					      TacsScalar stress[] ){
  int n = getNumStresses();
  for ( int i = 0; i < n; i++ ){
    stress[i] = 0.0;
  }
}
...
*/
void TACSConstitutive::addStressDVSens( const double pt[], const TacsScalar strain[], 
					TacsScalar alpha, const TacsScalar psi[], 
					TacsScalar dvSens[], int dvLen ){
  printf("addStressDVSens not implemented for %s\n",
	 this->TACSObjectName());
}

int TACSConstitutive::getNumMassMoments(){ return 1; }

/*
  Get the pointwise mass for the current constitutive relationship
  Here the array returned consists of at least the zeroth, and possible 
  higher moments of the density through the thickness of the element.
*/
void TACSConstitutive::pointwiseMass( const double gpt[], TacsScalar mass[] ){
  mass[0] = 0.0;
}

/*
  Return the sensitivity of the mass moments to the set design variable
*/
void TACSConstitutive::pointwiseMassDVSens( int dvIndex, const double gpt[],
					    TacsScalar massDVSens[] ){
  massDVSens[0] = 0.0;
}

void TACSConstitutive::addPointwiseMassDVSens( const double pt[], 
					       const TacsScalar alpha[],
					       TacsScalar dvSens[], int dvLen ){
  printf("addPointwiseMassDVSens not implemented for %s\n",
	 this->TACSObjectName());
}

/*
  Default implementations for the failure calculations. 
  
  These print out error messages to stderr if they are used.  The
  values from functions should not be trusted if errors are printed to
  the screen.
*/
void TACSConstitutive::failure( const double gpt[], 
				const TacsScalar strain[],
				TacsScalar * fail ){
  *fail = 0.0;
}

void TACSConstitutive::failureStrainSens( const double gpt[], 
					  const TacsScalar constStrain[],
					  TacsScalar failSens[] ){
  int n = getNumStresses();
  for ( int i = 0; i < n; i++ ){
    failSens[i] = 0.0;
...
void TACSConstitutive::failureDVSens( int dvIndex,
				      const double gpt[], 
				      const TacsScalar strain[],
				      TacsScalar * failSens ){
  *failSens = TacsScalar(0.0);
}

void TACSConstitutive::addFailureDVSens( const double pt[], 
					 const TacsScalar strain[],
					 TacsScalar alpha,
					 TacsScalar dvSens[], int dvLen ){
  printf("addFailureDVSens not implemented for %s\n",
	 this->TACSObjectName());
}

/*
  The default implementations for the buckling calculations. 
*/
void TACSConstitutive::buckling( const TacsScalar strain[],
				 TacsScalar * bval ){
  *bval = 0.0;
}

void TACSConstitutive::bucklingStrainSens( const TacsScalar strain[],
					   TacsScalar bvalSens[] ){
  int n = getNumStresses();
  for ( int i = 0; i < n; i++ ){
    bvalSens[i] = 0.0;
...
void TACSConstitutive::bucklingDVSens( int dvNum, 
				       const TacsScalar strain[],
				       TacsScalar * bvalSens ){
  *bvalSens = TacsScalar(0.0);
}

void TACSConstitutive::addBucklingDVSens( const TacsScalar strain[],
					  TacsScalar alpha,
					  TacsScalar dvSens[], int dvLen ){
  printf("addBucklingDVSens not implemented for %s\n",
	 this->TACSObjectName());
}
