...
@Date    :   2024/07/09
@Author  :   Alasdair Christison Gray, Sean Engelstad
@Description :
*Adapted Ali's example for comparison of our two buckling predictions.
A simple example of optimising a stiffened panel using the
...
stiffenerHeight = 17.9e-3
stiffenerHeightMin = 10e-3
stiffenerHeightMax = 0.15

stiffenerThickness = 3.59e-3
...
scenarioNames = ["CompAndShear"]
meshNames = {"CompAndShear": "plate.bdf"}
...
currentDVNum += 1
if args.usePlyFracDV:
    optSkinPlyFrac = dv_struct[currentDVNum : currentDVNum + 1]
    currentDVNum += 1
else:
...
currentDVNum += 1
if args.usePlyFracDV:
    optStiffPlyFrac = dv_struct[currentDVNum : currentDVNum + 1]
    currentDVNum += 1
else:
