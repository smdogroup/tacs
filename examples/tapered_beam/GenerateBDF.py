...
def getBeamSectionProperties(width: float, depth: float) -> Tuple[float, float, float, float]:
    """
...
def generateTaperedBeamBdf(numElements: int, propType: str, output: str) -> None:
    ...
    tipNodeId = nodeIds[-1]
    model.add_force(tipLoadId, tipNodeId, 1000.0, [0.0, 0.0, 1.0], comment="Vertical tip load of 1kN")
    ...
    sol101Path = outputStem.with_name(f"{outputStem.name}_{propType.lower()}_sol101.bdf")
    sol103Path = outputStem.with_name(f"{outputStem.name}_{propType.lower()}_sol103.bdf")
...
if __name__ == "__main__":
    ...
    generateTaperedBeamBdf(numElements=args.num_elements, propType=args.prop_type, output=args.output)
