...
*/
void setLinearSolution( TACSAssembler *assembler, TACSBVec *ans,
                        TacsScalar R, TacsScalar coef[] ){
  TACSBVec *X = assembler->createNodeVec();
  X->incref();
  assembler->getNodes(X);
...
int main( int argc, char *argv[] ){
  ...
  TACSElement *shell4 = new TACSQuad4Shell(transform, con);
  shell4->incref();

  TACSElement *shell9 = new TACSQuad9Shell(transform, con);
  shell9->incref();

  TACSElement *shell3 = new TACSTri3Shell(transform, con);
  shell3->incref();
  ...
  setExactSolution(assembler, ans, R, alpha, beta,
                   U, V, W, theta, phi);
