...
class AnalysisManager(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # design history file
        self._design_hdl = open(os.path.join(tacs_aim.analysis_dir, "design_hist.txt"), "w")
        ...
        for caseID in self.SPs:
            ...
            self.SPs[caseID].addFunction('ks_vmfailure', functions.KSFailure, safetyFactor=1.5, ksWeight=50)
    ...
    def compute(self, inputs, outputs):
        ...
        if not(design_change) and self._first_design:
            design_change = True; self._first_design = False
    ...
    def _print_design(self, inputs):
        ...
        self._design_hdl.write(f"\tthick dvs = {[_.name for _ in tacs_aim.thickness_variables]}\n")
    ...
    def _function_report(self):
        ...
        for func_name in self._func_names:
            self._design_hdl.write(f"\tfunc {func_name} = {self._func_history[func_name][-1]}\n")
    ...
    def compute_partials(self, inputs, partials):
        ...
        for func_name in self._func_names:
            for thick_var in tacs_aim.thickness_variables:
                partials[func_name, thick_var.name] = self._gradients[func_name][thick_var.name]
    ...
    def _run_analysis(self, inputs):
        ...
        for caseID in self.SPs:
            ...
            self.SPs[caseID].writeSolution(baseName="tacs_output", outputDir=tacs_aim.analysis_dir)
...
om_problem.driver.options['tol'] = 1.e-9
om_problem.driver.options['disp'] = True

# add design variables to the model
for thick_var in tacs_aim.thickness_variables:
    om_problem.model.add_design_var(f"tacsSystem.{thick_var.name}", lower=0.001, upper=0.1)
